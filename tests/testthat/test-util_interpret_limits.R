...
test_that("posix offset is 1970-01-01 01:00:00 CET)", {
  ...
  expect_equal(
    as.POSIXct(as.numeric(as.POSIXct("1975-12-16 03:46:00 CET")),
...
test_that("util_interpret_limits works", {
  ...
  meta[[WELL_KNOWN_META_VARIABLE_NAMES$HARD_LIMITS]][23] <- "(2020-01-01 00:00:00 CET; Inf]" # nolint: line_length_linter.
  meta[[WELL_KNOWN_META_VARIABLE_NAMES$HARD_LIMITS]][24] <- "(+Inf; 2019-09-09 00:00:00 CET]" # nolint: line_length_linter.
  meta[[WELL_KNOWN_META_VARIABLE_NAMES$HARD_LIMITS]][25] <- "(2020-01-01 00:00:00 CET; ]" # nolint: line_length_linter.
  meta[[WELL_KNOWN_META_VARIABLE_NAMES$HARD_LIMITS]][26] <- "(+Inf; ]"
  m2 <- util_interpret_limits(meta)
  a <- m2[, c(
    "HARD_LIMIT_LOW", "HARD_LIMIT_UP", "INCL_HARD_LIMIT_LOW",
    "INCL_HARD_LIMIT_UP"
  )]
  b <- dplyr::tribble(
