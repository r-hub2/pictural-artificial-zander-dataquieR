test_that("rbind.ReportSummaryTable keeps non-empty second table", {
  skip_on_cran()

  empty <- util_new_report_summary_table(data.frame())
  one_row <- util_new_report_summary_table(
    data.frame(Variables = "x", N = 1L)
  )

  expect_equal(rbind.ReportSummaryTable(empty, one_row), one_row)
})

test_that("rbind.ReportSummaryTable validates and returns local inputs", {
  skip_on_cran()
  ...
  expect_error(
    rbind.ReportSummaryTable(list(one_row)),
    regexp = "Can only bind ReportSummaryTables"
  )
  expect_equal(rbind.ReportSummaryTable(one_row), one_row)
})
...
test_that("rbind.ReportSummaryTable works", {
  ...
  expect_equal(
    ...
    util_new_report_summary_table(data.frame())
  )
  ...
  expect_equal(
    rbind.ReportSummaryTable(empty),
    util_new_report_summary_table(data.frame())
  )
  ...
  im2 <- suppressWarnings(suppressMessages(com_item_missingness(
    resp_vars = "DBP_0", study_data, md0,
    label_col = LABEL,
    suppressWarnings = TRUE,
    threshold_value = 100,
    include_sysmiss = FALSE,
  ...
  im3 <- suppressWarnings(suppressMessages(
    com_item_missingness(
      resp_vars = c("AGE_0", "AGE_1"), study_data, md0,
      label_col = LABEL,
      suppressWarnings = TRUE,
      threshold_value = 100,
      include_sysmiss = FALSE,
  ...
  expect_equal(
    ...
    structure(
      list(
        Variables = structure(c("AGE_0", "AGE_1"),
          label_col =
            "LABEL"
        ),
        N = c(2940L, 2940L)
