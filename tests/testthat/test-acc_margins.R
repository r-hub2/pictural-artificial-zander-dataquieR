test_that("acc_margins works without label_col", {
  ...
  expect_error(
    {
      res1 <-
        acc_margins(
          resp_vars = "v00001", study_data = study_data,
          meta_data = meta_data
        )
    },
    regexp = paste(
      ".*Argument .+resp_vars.+: Variable .+v00001.+ \\(na\\)",
  ...
  expect_error( # float in group_vars
    ...
    regexp = paste(
      "Argument .+group_vars.+: Variable .+CRP_0.+",
  ...
  expect_error( # wrong group_vars
    ...
    regexp = paste(
      "Variable .+not_avail.+ \\(group_vars\\) not found in",
  ...
  expect_error( # many group_vars
    ...
    regexp = paste(
      "Need exactly one element in argument group_vars,",
  ...
  expect_message2(
    ...
    regexp = paste(
      "Due to missing values in SEX_0, N = 60 observations",
  ...
  expect_message2( # util_margins_bin
    res2 <- acc_margins(
      resp_vars = "MEAT_CONS_0", study_data = study_data,
  ...
  expect_message2(
    res2b_bin <- acc_margins(
      resp_vars = "PREGNANT_0", study_data = study_data,
  ...
  sd1 <- study_data
  sd1[
    which(sd1[, md$VAR_NAMES[md$LABEL == "CENTER_0"]] == 1),
    md$VAR_NAMES[md$LABEL == "MEAT_CONS_0"]
  ] <- 0
  sd1[
    which(sd1[, md$VAR_NAMES[md$LABEL == "CENTER_0"]] == 4),
    md$VAR_NAMES[md$LABEL == "MEAT_CONS_0"]
  ] <- 1
  expect_message2( # util_margins_bin
    res2c <- acc_margins(
      resp_vars = "MEAT_CONS_0", study_data = sd1,
      meta_data = md, group_vars = "CENTER_0",
      label_col = LABEL, dichotomize_categorical_resp = TRUE
    )
  )
  ...
  res2d <- acc_margins(
    resp_vars = "MEAT_CONS_0", study_data = study_data, # util_margins_nom
    meta_data = md, group_vars = "CENTER_0",
  ...
  expect_message2( # util_margins_poi
    res4 <- acc_margins(
      resp_vars = "N_INJURIES_0", study_data = study_data,
      meta_data = md, group_vars = "USR_SOCDEM_0",
      label_col = LABEL, sort_group_var_levels = TRUE,
      include_numbers_in_figures = FALSE
    )
  ...
  expect_error(
    ...
    regexp = "Argument .group_vars.+Variable .v00001.+na. does not have an allowed scale level", # nolint: line_length_linter.
    perl = TRUE
  )
  ...
  expect_message2(
    ...
    regexp =
      paste(
        "min_obs_in_subgroup is not specified correctly",
  ...
  expect_message2(
    ...
    regexp =
      sprintf(
        "(%s|%s)",
        paste(
          "Due to missing values in v00016, N = 308",
  ...
  expect_message2(
    ...
    regexp =
      sprintf(
        ...
        paste(
          "Due to missing values.+308",
          "observations were excluded."
        ),
        paste(
          "Due to missing values.+131",
          "observations were excluded additionally."
        ),
  ...
  expect_message2(
    ...
    regexp =
      sprintf(
        "(%s|%s|%s)",
        paste(
          "No or many minimum observation count was specified and is set to n=5." # nolint: line_length_linter.
        ),
        paste("No co_vars specified"),
        paste(
          "Due to missing values in DEV_NO_0, N = 308",
          "observations were excluded. Due to missing values in",
  ...
  expect_message2(
    ...
    regexp =
      sprintf(
        ...
        paste(
          "Threshold was set to user but no value for the unit of",
...
test_that("acc_margins works with co_vars", {
  ...
  suppressMessages(
    res1_lm_unadj <- acc_margins(
      resp_vars = "SBP_0", # v00004
  ...
  expect_lte(
    max(abs(as.numeric(res1_lm_unadj$ResultData$margins) -
  ...
  sd1[["v00007"]] <- rbinom(n = nrow(sd1), size = 1, prob = 1 / (1 + exp(-mlin))) # nolint: line_length_linter.
  md1 <- meta_data
  md1[["RECODE_CASES"]] <- ""
  md1[["RECODE_CONTROL"]] <- ""
  ...
  expect_lte(
    max(abs(as.numeric(res5_poi_unadj$ResultData$margins) -
          as.numeric(util_round_to_decimal_places(as.vector(mu_emp))))),
    0.01
  )
  suppressMessages(
    res5_poi <- acc_margins(
      resp_vars = "N_INJURIES_0",
      group_vars = "USR_SOCDEM_0",
      co_vars = c("SEX_0", "AGE_0"),
      study_data = sd1,
  ...
  expect_lte(
    max(abs(as.numeric(res6_poi_unadj$ResultData$margins) -
          as.numeric(util_round_to_decimal_places(as.vector(mu_emp))))),
    0.01
  )
  suppressMessages(
    res6_poi <- acc_margins(
      resp_vars = "N_INJURIES_0",
      group_vars = "USR_SOCDEM_0",
      co_vars = c("SEX_0", "AGE_0"),
      study_data = sd1,
  ...
  sd1[["v00002"]][which(sd1[["v00002"]] == 1)] <-
    sample(c(0, 1),
      replace = TRUE,
      size = length(which(sd1[["v00002"]] == 1))
    ) # 0
  sd1[["v00002"]][which(sd1[["v00002"]] == 2)] <-
    sample(c(0, 1),
      replace = TRUE,
      size = length(which(sd1[["v00002"]] == 2))
    ) # 1
  ...
  expect_error(
    ...
    regexp = paste(
      "Adjusting for covariates is currently",
