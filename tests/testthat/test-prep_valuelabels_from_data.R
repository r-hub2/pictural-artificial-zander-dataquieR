test_that("prep_datatype_from_data works", {
  ...
  expect_message2(
    ...
    regexp =
      paste(
        "The following.+resp_vars.+are missing from the",
  ...
  expect_error(
    ...
    regexp =
      paste(
        ".+resp_vars.+should be missing or give variable",
...
test_that("prep_valuelabels_from_data keeps ordered factor ordering", {
  skip_on_cran()

  study_data <- data.frame(
    stage = ordered(c("low", "high"), levels = c("low", "high"))
  )

  value_labels <- prep_valuelabels_from_data(study_data)

  expect_identical(
    value_labels$VALUE_LABELS,
    c(stage = "1 = low < 2 = high")
  )
  expect_identical(value_labels$ModifiedStudyData$stage, c(1L, 2L))
})
