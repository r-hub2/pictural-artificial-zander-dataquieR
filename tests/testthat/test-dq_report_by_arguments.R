...
test_that("dq_report_by guards caller-owned clusters passed through dots", {
  ...
  testthat::local_mocked_bindings(
    util_guard_rstudio_user_cluster = function(cores, advanced_options) {
      ...
      stop("guarded before report_by computation", call. = FALSE)
    }
  )

  expect_error(
    dq_report_by(cores = fake_cl),
    "guarded before report_by computation"
  )
})
...
test_that("dq_report_by error_arguments_test", {
  ...
  expect_error(
    ...
    paste0(
      "selection_type can only be specified if strata_select",
      " or strata_exclude are present"
    ),
    fixed = TRUE
  )
  ...
  expect_error(
    ...
    paste0(
      "Internal error: The selection_type can only",
      ' be "value", "v_label", or "regex"'
    ),
    fixed = TRUE
  )
  ...
  expect_error(
    ...
    paste0(
      "Need exactly one element in argument subgroup, ",
  ...
  expect_error(
    ...
    regexp = ".+test/error.+does not exist. Provide an.+input_dir.+containing the study data" # nolint: line_length_linter.
  )
  ...
  expect_error(
    ...
    regexp = "The id_vars.+non-existing.+is not present in the item_level metadata" # nolint: line_length_linter.
  )
  ...
  expect_error(
    ...
    regexp = "The strata_column provided does not correpond.+item_level_metadata" # nolint: line_length_linter.
  )
  ...
  expect_error(
    ...
    regexp = "No metadata attribute.+STUDY_SEGMENT.+found for segmenting DQ report." # nolint: line_length_linter.
  )
  ...
  expect_message2(dq_report_by(
    study_data = c(
      "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/ship.RDS",
      "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/study_data.RData" # nolint: line_length_linter.
    ),
    meta_data_v2 = "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/meta_data_v2.xlsx", # nolint: line_length_linter.
  ...
  df3 <- prep_get_data_frame(
    "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/meta_data_v2.xlsx | dataframe_level" # nolint: line_length_linter.
  )
  ...
  segl3 <- prep_get_data_frame(
    "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/meta_data_v2.xlsx | segment_level" # nolint: line_length_linter.
  )
  ...
  df3$DF_NAME <- "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/study_data.RData" # nolint: line_length_linter.
  expect_message2(dq_report_by(
    meta_data = md3,
    meta_data_dataframe = df3,
    cores = NULL,
  ...
  df3 <- rbind(
    ...
    data.frame(
      ...
      DF_UNIQUE_ROWS = NA,
      DF_CODE = "ship",
      DF_NAME = "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/ship.RDS" # nolint: line_length_linter.
    )
  )
