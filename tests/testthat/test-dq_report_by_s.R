test_that("dq_report_by works with content s", {
  ...
  sd0 <- head(prep_get_data_frame("https://dataquality.qihs.uni-greifswald.de/extdata/fortests/study_data.RData", keep_types = TRUE), 100) # nolint: line_length_linter.
...
test_that("dq_report_by works with complex limits", {
  ...
  sd0 <- head(prep_get_data_frame("https://dataquality.qihs.uni-greifswald.de/extdata/fortests/study_data.RData", keep_types = TRUE), 50) # nolint: line_length_linter.
  ...
  expect_message2(dq_report_by(sd0,
      ...
      filter_indicator_functions = "con_hard_limits",
