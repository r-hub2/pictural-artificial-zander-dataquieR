test_that("prep_datatype_from_data works", {
  skip_on_cran()
  expect_equal(
    prep_datatype_from_data(cars),
    c(speed = "integer", dist = "integer")
  )
  expect_equal(
    prep_datatype_from_data("speed", cars),
    c(speed = "integer")
  )
  expect_message2(
    expect_equal(
      prep_datatype_from_data("speedx", cars),
      c(speedx = NA_character_)
    ),
    regexp = paste(
      "The following .+resp_vars.+ are missing from",
      "the .+study_data.+.",
...
test_that("prep_datatype_from_data works with tibble", {
  ...
  expect_equal(
    prep_datatype_from_data(tibble::as_tibble(cars)),
    c(speed = "integer", dist = "integer")
  )
})
