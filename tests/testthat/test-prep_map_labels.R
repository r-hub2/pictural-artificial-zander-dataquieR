test_that("prep_map_labels works", {
  ...
  expect_equal(
    prep_map_labels(c("AGE", "DOE"), meta_data),
    c(AGE = "Age", DOE = "Examination Date")
  )
  expect_equal(
    prep_map_labels(c("AGE", "DOE"), item_level = meta_data),
    c(AGE = "Age", DOE = "Examination Date")
  )
  expect_equal(prep_get_labels(c("AGE", "DOE"), item_level = meta_data),
    c(AGE = "Age", DOE = "Examination Date"),
    ignore_attr = TRUE
  )
  expect_error(
    prep_get_labels("AGE",
      ...
      label_col = "MISSING_LABEL_COLUMN"
    ),
    regexp = "needs to exist"
  )
  expect_error(
    prep_map_labels("AGE",
      item_level = meta_data,
    ...
    regexp = "You cannot provide both"
  )
  expect_error(
    prep_get_labels("AGE",
      item_level = meta_data,
  ...
  expect_equal(
    prep_map_labels(c("AGE", "NOT_AVAIL"), meta_data,
  ...
  expect_error(
    prep_map_labels(
      x = meta_data$VAR_NAMES,
      meta_data = meta_data,
      to = 42
    ),
    regexp =
      paste(
        "Need exactly one existing variable",
