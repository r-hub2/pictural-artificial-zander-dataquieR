test_that("util_warn_unordered works", {
  ...
  expect_warning(util_warn_unordered(factor(c("low", "high")), "rating"),
    perl = TRUE,
    regexp =
      "Don't know, how to compare values of .+rating.+ (.+factor.+)"
  )
  expect_silent(util_warn_unordered(ordered(c("low", "high")), "rating"))
