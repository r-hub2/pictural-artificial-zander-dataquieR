...
util_find_var_by_meta <- function(resp_vars,
  ...
  if (!(length(ifnotfound) %in% c(1, length(resp_vars)))) {
    util_error(
      "%s must have either length 1 or the same length as %s",
  ...
  # have not changed enough here to justify re-preparing on every lookup.
  ...
  # check whether the user-specified variable could not be found in any of the
  # other columns:
  map_res_na <- vapply(
