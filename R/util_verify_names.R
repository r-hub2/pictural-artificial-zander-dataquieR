...
#' Internal helper: util verify names
#'
#' @noRd
.util_verify_names <- function(standard_names = c(
  "CODE_LIST_TABLE",
  "MISSING_CODE_RULES",
  "cross-item_level",
  "dataframe_level",
...
name_of_study_data = character(0)) {
  observed_names <- setdiff(
    ...
    c(
      ...
      "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/meta_data.RData", # nolint: line_length_linter.
      "https://dataquality.qihs.uni-greifswald.de/extdata/fortests/study_data.RData" # nolint: line_length_linter.
    )
  )
  observed_names <- observed_names[!startsWith(
    observed_names,
  ...
  while (len_ref_nm < length(referred_names)) {
    ...
    referred_names_df <- lapply(observed_names, function(x) {
      r <- NULL
      try(r <- prep_get_data_frame(x), silent = TRUE)
      r
    })
    ...
    referred_names_cur <- lapply(referred_names_df, function(x) {
      if (is.data.frame(x) && ncol(x) > 0) {
        x[, endsWith(colnames(x), "_TABLE"), drop = FALSE]
      } else {
        character(0)
      }
    })
    # update referred_names
    referred_names <- unique(c(
      referred_names,
  ...
  referred_names <- c(referred_names, vapply(prefixes,
    FUN = function(prefix) {
  ...
  # try to compare expected names with names available, warn about data frames
  # not expected
  ...
  names(dimnames(d1)) <- names(dimnames(d2)) <- c("standard_names", "observed_names") # nolint: line_length_linter.
  ...
  if (util_ensure_suggested("stringdist", err = FALSE, goal = "guessing typos")) { # nolint: line_length_linter.
  ...
  # compute the minimum distance to an expected name for each column (observed
  # names)
  ...
  if (!is.null(dontknow)) {
    for (sn in c(standard_names, observed_names)) {
      dontknow <- dontknow[!endsWith(dontknow, paste0("|", sn))]
    }
  }
  list(
    case_insensitive = d1, case_sensitive = d2, mismatches = non_standard,
