...
util_get_encoding <- function(resp_vars = colnames(study_data),
  ...
  if (ENCODING %in% colnames(meta_data)) {
    ...
    if (length(unkown) > 0) {
      util_warning(
        "Found unkown encodings in metadata: %s",
  ...
  if (DATAFRAMES %in% colnames(meta_data) &&
    ...
    meta_data_dataframe[[ENCODING]][meta_data_dataframe[[ENCODING]] == ""] <- NA_character_ # nolint: line_length_linter.
    rvs <- resp_vars[is.na(result[resp_vars])]
    ...
    result[rvs] <-
      vapply(rvs, function(rv) {
        ...
        if (length(enc) > 1) {
          util_warning(
            c(
              "Found more than one encodings for variable %s",
  ...
  if (length(rvs) > 0) {
    util_warning(
      c(
        "Encoding information neither in item-",
        "nor dataframe-level-metadata found for %s, guessing from",
        "data."
      ),
      util_pretty_vector_string(rvs),
      applicability_problem = TRUE
    )
  }
  if (length(rvs) > 0 && !util_stringi_available()) {
    util_warning(
      c(
        "Cannot guess encodings without the optional package",
        "%s; assuming UTF-8 for %s."
      ),
      sQuote("stringi"),
      util_pretty_vector_string(rvs),
      applicability_problem = TRUE,
