...
util_create_mahalanobis_ggplot <- function(md_ratio,
  ...
  # Historical color-only outlier encoding removed here.
  ...
  # (same no. points as the sample and degree of freedom equal to the no.
  # variables)
  probabilities <- stats::ppoints(length(md_ratio))
  theoretical_quantiles <- stats::qchisq(probabilities, df = df)

  # Obtain threshold to identify multivariate outliers (argument
  # mahalanobis_threshold)
  # it is by default 0.975 quantile of a chi-square distribution
  # with p degrees of freedom, where p = variables (Mayrhofer and Filzmoser,
  # 2023)
  ...
  # order the squared Mahal. distance, actually not needed, as ordered
  # before----
  # Historical duplicate MD_values ordering removed here.

  # create a column with identified outliers based on threshold----
  plot_data$MD_outliers <- NA
  plot_data$MD_outliers <- ifelse(plot_data$MD_values > md_outliers_threshold, 1, 0) # nolint: line_length_linter.
  ...
  p1 <- util_create_lean_ggplot(
    {
      ggplot(plot_data, aes(
        ...
        geom_hline(
          yintercept = md_outliers_threshold,
          linetype = "dashed",
          color = "black",
          linewidth = 0.6
        ) +
