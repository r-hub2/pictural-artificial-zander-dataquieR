...
prep_get_labels <- function(
  ...
  max_len_missing <- missing(max_len)
  if (max_len_missing) {
    ...
    max_len <- .util_validate_label_length_limit(max_len,
      argument = "max_len"
    )
  }
  ...
  if (resp_vars_match_label_col_only &&
    ...
    util_error(
      "Call with either %s or %s set TRUE, not both",
      sQuote("resp_vars_match_label_col_only"),
      sQuote("resp_vars_match_label_col_only")
    )
  }
  ...
  if (!no_lc && !label_col %in% colnames(meta_data)) {
    util_error(
      "%s = %s, if specified, needs to exist in %s.",
      sQuote("label_col"),
      dQuote(label_col),
      sQuote("meta_data")
    )
  ...
  to <- c(paste(to, label_lang, sep = "_"), to) # see what happens with label_lang # nolint: line_length_linter.
  ...
  uel <- .util_ensure_label(
    meta_data[
      meta_data[[VAR_NAMES]] %in% vns,
      c(VAR_NAMES, to), drop = FALSE
    ],
    label_col = to,
    max_label_len = max_len
  )

  res <- prep_map_labels( # do not attach "to" as the used label col if the follwoing fails or vn was already not a. # nolint: line_length_linter.
    x = vns,
