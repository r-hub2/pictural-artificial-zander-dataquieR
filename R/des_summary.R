...
#'
#' @details
#' If `resp_vars` is missing or empty, all variables in the prepared study data
#' are selected. Selected variables may include multiple variables, but each
#' selected variable must have a defined scale level in the metadata.
#'
...
des_summary <- function(resp_vars = NULL,
  ...
  if (!VARIABLE_ORDER %in% colnames(meta_data)) {
    # VARIABLE_ORDER missing, add appearance order of variables in the study
    # data
  ...
  # Remove variables with missing scale levels.
  ...
  md_filtered <- meta_data[meta_data[, label_col, drop = TRUE] %in%
      resp_vars, , drop = FALSE]
  md_filtered$nprog <- seq_len(nrow(md_filtered))
  ...
  categorical_cols <- md_filtered[[SCALE_LEVEL]] %in% c(
  ...
  ratio_interv_cols <- md_filtered[[SCALE_LEVEL]] %in% c(
    SCALE_LEVELS$RATIO,
  ...
  rev_summary_labels <- cbind(
    rev_summary_labels,
    vapply(
      apply(rev_summary_labels, 1,
        unique,
        simplify = FALSE
      ),
      paste,
  ...
  variable_names <- setNames(
    nm = rev_summary_labels[[label_col]],
  ...
  summary_table$Variable_names <- util_attach_attr(
    variable_names[match(
      summary_table[[label_col]],
  ...
  summary_table$mean_contin <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$mean_datetime <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table <- summary_table[, cols_to_keep, drop = FALSE]
  ...
  summary_table$median_cat <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$median_contin <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$mode_cat <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$mode_contin <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$mode_datetime <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$SD_contin <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  vec1 <-
    unlist(lapply(
      ...
      function(x) {
        ...
        # formatted_time <-
        # util_des_functions_env$util_format_duration_human(start =
        # total_seconds)
        formatted_time <-
  ...
  summary_table$SD_datetime <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$IQR_ord <- iqr[match(
    summary_table[[label_col]],
    names(iqr)
  )]
  summary_table$IQR_ord_q1_q3 <- iqr_q1_q3[match(
    summary_table[[label_col]],
    names(iqr_q1_q3)
  )]
  # create empty one in case not created
  ...
  rm(vec1)
  # Add both to summary_table
  summary_table$IQR_cont <- iqr[match(
    summary_table[[label_col]],
    names(iqr)
  )]
  summary_table$IQR_cont_q1_q3 <- iqr_q1_q3[match(
    summary_table[[label_col]],
    names(iqr_q1_q3)
  )]
  # create empty one in case not created
  ...
  rm(vec1)
  # Add both to summary_table
  summary_table$IQR_datetime <- iqr[match(
    summary_table[[label_col]],
    names(iqr)
  )]
  ...
  summary_table$MAD_contin <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$MAD_datetime <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  if (!is.null(vec1)) {
    summary_table$min_ord <- vec1[match(
      summary_table[[label_col]],
      names(vec1)
    )]
    rm(vec1)
  ...
  if (!is.null(vec1)) {
    summary_table$max_ord <- vec1[match(
      summary_table[[label_col]],
      names(vec1)
    )]
    rm(vec1)
  ...
  if (!is.null(vec1)) {
    summary_table$min_contin <- vec1[match(
      summary_table[[label_col]],
      names(vec1)
    )]
    rm(vec1)
  ...
  if (!is.null(vec1)) {
    summary_table$max_contin <- vec1[match(
      summary_table[[label_col]],
      names(vec1)
    )]
    rm(vec1)
  ...
  summary_table$range_temp <- summary_table$max_contin - summary_table$min_contin # nolint: line_length_linter.
  ...
  summary_table$min_max_contin[result] <-
    paste0(
      summary_table$range_temp[result],
      " (",
  ...
  if (!is.null(vec1)) {
    summary_table$min_datetime <- vec1[match(
      summary_table[[label_col]],
      names(vec1)
    )]
    rm(vec1)
  ...
  if (!is.null(vec1)) {
    summary_table$max_datetime <- vec1[match(
      summary_table[[label_col]],
      names(vec1)
    )]
    rm(vec1)
  ...
  vec1 <-
    unlist(lapply(
      ...
      function(x) {
        ...
        # Historical lubridate interval formatting removed here.
  ...
  if (!is.null(vec1)) {
    summary_table$min_max_datetime <- vec1[match(
      summary_table[[label_col]],
      names(vec1)
    )]
    rm(vec1)
  ...
  summary_table$skewness_value <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  if (!"skewness_value" %in% colnames(summary_table)) {
    ...
    summary_table$skewness_value[!no_result] <-
      format(util_round_to_decimal_places(summary_table$skewness_value[!no_result])) # nolint: line_length_linter.
  ...
  summary_table$SE_skewness <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  if (!"SE_skewness" %in% colnames(summary_table)) {
    ...
    summary_table$SE_skewness[!no_result] <-
      format(util_round_to_decimal_places(summary_table$SE_skewness[!no_result])) # nolint: line_length_linter.
  ...
  if ("skewness_value" %in% colnames(summary_table) &&
    ...
    # merge skewness
    no_result <- is.na(summary_table$skewness_value) | summary_table$skewness_value == "" # nolint: line_length_linter.
  ...
  summary_table$Kurtosis <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$Level_freq <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$valid_N <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$valid_PCT <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$missing_N <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$missing_PCT <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$graph_cat <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$graph_datetime <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  summary_table$graph_cont <- vec1[match(
    summary_table[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  ...
  # Additional column to summary_table to make it compatible with previous
  # version
  # Type----
  vec_type <- setNames(
    nm = summary_table[[label_col]],
    paste0(
      summary_table$SCALE_LEVEL,
      ", ", summary_table$DATA_TYPE
    )
  )
  summary_table$Type <- vec_type[match(
    summary_table[[label_col]],
    names(vec_type)
  )]
  ...
  summary_data <- summary_data[, !(colnames(summary_data) %in%
        c(SCALE_LEVEL, DATA_TYPE)), drop = FALSE]
  ...
  summary_data$"Frequency table" <- vec1[match(
    summary_data[[label_col]],
    names(vec1)
  )]
  rm(vec1)
  # create empty one in case not created
  ...
  names(summary_data)[names(summary_data) == "No_categories_incl_NA"] <- "No. categories (incl.NAs)" # nolint: line_length_linter.
  # replace NA in summary_data with ""
  summary_data[is.na(summary_data)] <- ""


  return(list(
    SummaryData = util_attach_attr(summary_data,
