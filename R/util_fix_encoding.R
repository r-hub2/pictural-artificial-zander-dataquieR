...
util_fix_encoding <- function(x) {
  byte_length <- nchar(x, type = "bytes", allowNA = TRUE, keepNA = TRUE)
  char_length <- nchar(x, type = "chars", allowNA = TRUE, keepNA = TRUE)
  known_ascii <- !is.na(byte_length) & !is.na(char_length) &
    byte_length == char_length
  if (all(is.na(x) | known_ascii)) {
    return(x)
  }
