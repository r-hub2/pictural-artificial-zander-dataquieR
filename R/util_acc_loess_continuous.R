...
util_acc_loess_continuous <- function(
  ...
  has_dummy_group <- is.null(group_vars) || all(util_empty(group_vars))
  if (has_dummy_group) {
  ...
  n_prior <- nrow(ds1)
  ds1 <- ds1[, c(resp_vars, time_vars, group_vars, co_vars), drop = FALSE]
  if (grepl("dummy_group", group_vars)) {
    ...
    if (any(is.na(ds1[
      complete.cases(ds1[, c(time_vars, co_vars), drop = FALSE]),
      group_vars
      , drop = TRUE]))) {
  ...
  ds1 <- ds1[complete.cases(ds1[, c(time_vars, group_vars, co_vars), drop = FALSE]), , drop = FALSE] # nolint: line_length_linter.
  n_post <- nrow(ds1)
  msg <- NULL
  if (n_post < n_prior) {
    msg <- paste0(
  ...
  tab_groups_tp <- vapply(levels(ds1[[group_vars]]),
    FUN.VALUE = numeric(1),
    FUN = function(gr) {
  ...
  ds1 <- ds1[order(ds1[[time_vars]], ds1[[group_vars]]), , drop = FALSE]
  ...
  if (length(co_vars) > 0) {
    model_terms <- util_bQuote(co_vars)
    if (!has_dummy_group) {
      model_terms <- c(model_terms, util_bQuote(group_vars))
    }
    ...
    if (has_dummy_group) {
      ds1$Residuals <- mean(ds1[[resp_vars]], na.rm = TRUE) + lmfit1$residuals
    } else {
      ...
      ds1$Residuals <-
        ...
        group_marg$emmean[match(ds1[[group_vars]], group_marg[, group_vars, drop = TRUE])] + # nolint: line_length_linter.
  ...
  processed_grouped_ds1 <- lapply(grouped_ds1, function(data_i) {
    if (var(data_i[["Residuals"]]) == 0) { # constant for this subgroup
      df_i <- unique(data_i[, c("Residuals", "ROUND_TIME"), drop = FALSE])
    ...
    return(res_round_tp = pred_df[which(!is.na(pred_df$FITTED_VALUE)), , drop = FALSE]) # nolint: line_length_linter.
  })
  ...
  if (any(compl)) {
    if (!compl[1]) {
      global_trends[1:(which(compl)[1] - 1), ] <- global_trends[which(compl)[1], , drop = FALSE] # nolint: line_length_linter.
    }
  ...
  if (any(compl)) {
    if (lines_to_add == "mean/sd") {
      lines_info <- paste(
        "Trend lines shown for comparison indicate mean \u00B1",
        sd_fac, "SD."
      )
    } else {
      lines_info <- paste(
        "Trend lines shown for comparison indicate",
        "quartiles Q1, Q2, Q3."
      )
    }
  } else {
  ...
  if (mark_time_points || plot_observations) {
    sel_obs <- list(
      ...
      "dp_facets" = seq_len(nrow(ds1)), # for facets
      "dp_comb" = seq_len(nrow(ds1)), # for combined plot
      # Scatter plot of observed values:
      "obs_facets" = seq_len(nrow(ds1)), # for facets
      "obs_comb" = seq_len(nrow(ds1))
    ) # for combined plot
    ...
    if (nrow(ds1) > points_shown_max) {
      ...
      sel_obs$obs_facets <- sort(unlist(
        lapply(levels(ds1[[group_vars]]), function(gr) {
          if (sum(ds1_tab[gr, , drop = FALSE]) > points_shown_max) {
            ds1_tab_gr_adj <- floor(ds1_tab[gr, , drop = FALSE] / sum(ds1_tab[gr, , drop = FALSE]) * # nolint: line_length_linter.
                points_shown_max)
            unlist(lapply(names(ds1_tab_gr_adj), function(tp) {
              ...
              if (length(gr_tp_ind) == 0 || ds1_tab_gr_adj[tp] == 0) {
                out <- integer(0)
              } else if (var(ds1[gr_tp_ind, "Residuals", drop = TRUE]) == 0) {
                out <- sample(gr_tp_ind, size = ds1_tab_gr_adj[tp])
              } else {
                # keep 'extreme' values to show the true range
                gr_tp_max <- gr_tp_ind[which.max(ds1[gr_tp_ind, "Residuals", drop = TRUE])] # nolint: line_length_linter.
                gr_tp_min <- gr_tp_ind[which.min(ds1[gr_tp_ind, "Residuals", drop = TRUE])] # nolint: line_length_linter.
        ...
        lapply(levels(ds1[[group_vars]]), function(gr) {
          unlist(
            lapply(colnames(ds1_tab_adj), function(tp) {
              gr_tp_ind <- intersect(gr_ind[[gr]], tp_ind[[tp]])
              if (length(gr_tp_ind) == 0 || ds1_tab_adj[gr, tp, drop = TRUE] == 0) { # nolint: line_length_linter.
                out <- integer(0)
              } else if (var(ds1[gr_tp_ind, "Residuals", drop = TRUE]) == 0) {
                out <- sample(gr_tp_ind, size = ds1_tab_adj[gr, tp, drop = TRUE]) # nolint: line_length_linter.
              } else {
                # keep 'extreme' values to show the true range
                gr_tp_max <- gr_tp_ind[which.max(ds1[gr_tp_ind, "Residuals", drop = TRUE])] # nolint: line_length_linter.
                gr_tp_min <- gr_tp_ind[which.min(ds1[gr_tp_ind, "Residuals", drop = TRUE])] # nolint: line_length_linter.
                if (ds1_tab_adj[gr, tp, drop = TRUE] <= 2) {
                  out <- sample(c(gr_tp_max, gr_tp_min),
                    size = ds1_tab_adj[gr, tp, drop = TRUE]
                  )
                } else {
                  out <- c(
                    gr_tp_max, gr_tp_min,
                    sample(setdiff(gr_tp_ind, c(gr_tp_max, gr_tp_min)),
                      size = ds1_tab_adj[gr, tp, drop = TRUE] - 2
                    )
                  )
                }
              }
      ...
      sel_obs$dp_facets <- sel_obs$obs_facets[
        which(!duplicated(ds1[sel_obs$obs_facets, c(group_vars, time_vars), drop = FALSE])) # nolint: line_length_linter.
      ]
      sel_obs$dp_comb <- sel_obs$obs_comb[
        which(!duplicated(ds1[sel_obs$obs_comb, c(group_vars, time_vars), drop = FALSE])) # nolint: line_length_linter.
      ]
    }
  }
  ...
  if (mark_time_points) {
    ...
    mark_obs <- ds1[sel_obs_dp, c(time_vars, group_vars), drop = FALSE]
          ...
          if (tp %in% pred_gr$TIME) {
            ...
            if (length(tp_ind1) == 0 && length(tp_ind2) == 0) {
              ...
              local_lm <- lm(FITTED_VALUE ~ TIME,
                data = pred_gr[tp_ind1:tp_ind2, , drop = FALSE]
              )
    ...
    geom_dp_facets <- geom_point(
      shape = "|",
      data = mark_obs[mark_obs$facets == 1, c("TIME", "GROUP", "FITTED_VALUE"), drop = FALSE] # nolint: line_length_linter.
    )
    geom_dp_comb <- geom_point(
      shape = "|",
      data = mark_obs[mark_obs$comb == 1, c("TIME", "GROUP", "FITTED_VALUE"), drop = FALSE] # nolint: line_length_linter.
    )
  }
  ...
  if (plot_observations) {
    scatter_obs_facets <- ds1[
      sel_obs$obs_facets,
      c(time_vars, "Residuals", group_vars)
      , drop = TRUE]
    colnames(scatter_obs_facets) <- c("TIME", "FITTED_VALUE", "GROUP")
    geom_obs_facets <- geom_point(
      data = scatter_obs_facets,
      alpha = 0.2
    )
    scatter_obs_comb <- ds1[
      sel_obs$obs_comb,
      c(time_vars, "Residuals", group_vars)
      , drop = TRUE]
    colnames(scatter_obs_comb) <- c("TIME", "FITTED_VALUE", "GROUP")
    geom_obs_comb <- geom_point(
      data = scatter_obs_comb,
      alpha = 0.2
    )
  }
  ...
  if (any(compl)) {
    p1 <- util_create_lean_ggplot(
      p1 +
        geom_line(
          data = global_trends, aes(y = .data$low, group = NA),
          color = lines_arg$color, linetype = lines_arg$linetype
        ) +
        geom_line(
          data = global_trends, aes(y = .data$mid, group = NA),
          color = lines_arg$color, linetype = 1
        ) +
        geom_line(
          data = global_trends, aes(y = .data$high, group = NA),
          color = lines_arg$color, linetype = lines_arg$linetype
        ),
      p1 = p1,
  ...
  if (!is.null(pl[["Loess_fits_combined"]])) {
    ...
    min_point_line <- min(util_rbind(data_frames_list = obj1$data)$y, na.rm = TRUE) # nolint: line_length_linter.
    max_point_line <- max(util_rbind(data_frames_list = obj1$data)$y, na.rm = TRUE) # nolint: line_length_linter.
  ...
  if (plot_format == "BOTH") {
    ...
    return(util_attach_attr(
      ...
      sizing_hints = list(
        ...
        no_char_y = max(nchar(c(
          round(max_point_line, digits = 2),
  ...
  # Historical level-count based loess output selection removed here.
...
#' Internal helper: for moving window
#'
#' @noRd
