...
util_optimize_histogram_bins <- function(x, interval_freedman_diaconis = NULL,
  ...
  util_expect_scalar(x,
    allow_more_than_one = TRUE, allow_na = TRUE,
  ...
  if (is.null(interval_freedman_diaconis)) {
    ...
    interval_freedman_diaconis <-
      redcap_env$interval(
        inc_l = TRUE, inc_u = TRUE,
        low = min_x, upp = max_x
      )
  } else {
    if (!("interval" %in% class(interval_freedman_diaconis))) { # TOFIX: if the interval is 110; Inf it stops with an error # nolint: line_length_linter.
      interval_freedman_diaconis <-
    ...
    cuts <- c(
      cuts, interval_freedman_diaconis$low,
  ...
  if (length(cuts) > 1) {
    ...
    cuts <- cuts[c(TRUE, diff(cuts) > .Machine$double.eps)]
  }
  if (length(cuts) > 0) {
    # Every finite observation must be covered by the generated breaks. Do not
    # use a tolerance here: computed values can lie only a rounding error
    # outside a cut and still form a genuine below/above-limit segment.
    if (cuts[1] > min_x) {
      cuts <- c(-Inf, cuts)
    }
    if (max_x > cuts[length(cuts)]) {
      cuts <- c(cuts, Inf)
    }
  ...
  #   two times the IQR divided by the cubic root of n
  iqr_bw <- IQR(x_within)
  ...
  # - check whether the bandwidth `by_x` is not below the minimum bandwidth.
  ...
  #   bandwidth `by_x` is an integer >= 1
  if (all(util_is_integer(x))) {
    by_x <- max(round(by_x), 1)
  }
  ...
  if (length(cuts) > 1) {
    by_x_mean <- dif_seq / nbins_seq
    by_x_mean <- by_x_mean[is.finite(by_x_mean)]
    if (length(by_x_mean) > 0) {
      by_x_mean <- sum(by_x_mean) / length(by_x_mean)
      if (by_x_mean > bw_min) {
        by_x <- by_x_mean
      }
    }
    if (is.infinite(nbins_seq[1])) {
      nbins_seq[1] <- ceiling(abs(cuts[2] - min_plot) / by_x)
    }
    if (is.infinite(nbins_seq[length(nbins_seq)])) {
      nbins_seq[length(nbins_seq)] <-
        ceiling(abs(max_plot - cuts[length(cuts) - 1]) / by_x)
    }
  }
  ...
  if (length(cuts) <= 1) {
    ...
    for (i in seq_along(nbins_seq)) {
      if (is.infinite(cuts[i])) {
        ...
        # be infinite.
        all_breaks[[i]] <- seq(
          from = cuts[i + 1] - nbins_seq[i] * by_x,
          to = cuts[i + 1],
