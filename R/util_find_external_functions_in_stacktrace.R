...
util_find_external_functions_in_stacktrace <-
  function(sfs = rev(sys.frames()),
    cls = rev(sys.calls())) {
    is_me <-
      vapply(sfs, rlang::env_inherits,
        ancestor = parent.env(environment()),
        FUN.VALUE = logical(1)
      )

    # Historical parent-frame walk replaced by rlang::env_inherits().
    is_base <- vapply(sfs, identical, asNamespace("base"),
      FUN.VALUE =
        logical(1)
    ) # base never calls me, but by using do.call
    ...
    } else { # base never calls me, but by using do.call
      is_parallel <- is_base
    }
