...
#' @noRd
# nolint end
util_margins_nom <- function(resp_vars = NULL, group_vars = NULL, co_vars = NULL, # nolint: line_length_linter.
  ...
  if (length(co_vars) > 0) {
    ...
    covar_fact <- setNames(
      nm = co_vars,
      vapply(ds1[, co_vars, drop = FALSE],
        FUN.VALUE = logical(1),
        FUN = is.factor
      )
    )
    covar_excl <- union(
      which(covar_prop$NCategory <= 2),
      which(!covar_fact)
    )
    if (length(covar_excl) > 0) {
      util_error(paste(
  ...
  res_df <- merge(res_df, summary_ds,
    by = c(group_vars, resp_vars),
    all.x = TRUE
  )
  res_df <- dplyr::rename(res_df, c(
    "margins" = "prob", "LCL" = "lower.CL",
    "UCL" = "upper.CL"
  ))
  ...
  nom_test <- merge(res_df, omv[, c(resp_vars, "margins"), drop = FALSE],
    by = resp_vars, all = TRUE, suffixes = c("", ".overall")
  )
  test_df <- nom_test[, c(resp_vars, group_vars), drop = FALSE]
  ...
  res_df <- merge(res_df, test_df[, c(resp_vars, group_vars, "GRADING"), drop = FALSE], # nolint: line_length_linter.
  ...
  nom_plot_data <- merge(res_df, omv,
    by = c(
      resp_vars, group_vars, "margins", "SE", "df",
      "LCL", "UCL"
    ),
    all = TRUE
  )
  ...
  nom_plot_data[["facet"]] <- nom_plot_data[[resp_vars]] # make facet findable to prevent util_compress_ggplots_in_res from deleting this column # nolint: line_length_linter.

  res_plot <- util_create_lean_ggplot(
    ggplot(
      nom_plot_data,
      aes(
        x = .data[["margins"]],
