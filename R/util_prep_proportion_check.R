...
util_prep_proportion_check <- function(resp_vars,
  ...
  label_col = util_attr(ds1, "label_col",
    exact = TRUE
  )) {
  ...
  if (!isTRUE(util_attr(ds1, "apply_fact_md_inadm", exact = TRUE))) {
    util_error(
      paste(
        "Internal error, sorry, please report: proprotion check called with",
        "bare study data or without inadmissible values."
      )
    )
  }
  ...
  if (VALUE_LABELS %in% colnames(meta_data)) {
    util_error("Internal error, sorry, please report: VALUE_LABELS in util_prep_proportion_check.") # nolint: line_length_linter.
  }
  if (VALUE_LABEL_TABLE %in% colnames(meta_data)) {
    val_label_tables <- setNames(
      util_find_var_by_meta(
        resp_vars = resp_vars,
  ...
  prop_range_int <-
    lapply(setNames(nm = resp_vars), function(rv) {
      ...
      if (all(is.na(range_per_cat))) {
        ...
        # Leave range_per_cat missing for invalid categorical ranges.
      }
      range_per_cat
    })
