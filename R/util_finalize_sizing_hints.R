...
util_finalize_sizing_hints <- function(sizing_hints) {
  ...
  normalize_unit <- function(css_value) {
    ...
    for (.unit in .units) {
      ...
      while (!ready) {
        ...
        if (!ready) {
          e <- matches[[1]]
          e_length <- util_attr(e, "match.length", exact = TRUE)
          new_val <- substr(..v, e, e + e_length - 1)
          match <- regexec(pattern, new_val)[[1]]
          match_length <- util_attr(match, "match.length", exact = TRUE)
          from_number <- match[2]
          to_number <- from_number + match_length[2]
          from_unit <- match[3]
          to_unit <- from_unit + match_length[3]
          number <- as.numeric(substr(
            new_val, from_number, to_number - 1
          ))
          unit <- .unit
          if (unit == "px") { # assume pixels according to https://www.w3schools.com/cssref/css_units.php # nolint: line_length_linter.
            if (!is_sreen_rel) {
              util_message(c(
                "Internal error, sorry: dataquieR",
          ...
          if (suppressWarnings(
            util_ensure_suggested("units", err = FALSE)
          )) {
            number <- units::set_units(number, unit, mode = "standard")
            cm <- "dummy" # avoid notes about cm
            units(number) <- units::make_units(cm)
            number_cm <- units::drop_units(number)
          } else {
            # Fallback without the optional 'units' package: at this point,
            # 'unit' is always one of cm/mm/in (all other CSS units have
            # already been converted to 'in' above), so a simple linear
            # conversion to cm is sufficient and exact.
            number_cm <- switch(unit,
              "cm" = number,
              "mm" = number / 10,
              "in" = number * 2.54,
              NA_real_
            ...
            if (is.na(number_cm)) {
              util_error(
                c(
                  "Internal error in util_finalize_sizing_hints: cannot",
                  "convert unit %s to cm without the optional 'units'",
    ...
    if (length(..v) != 1 ||
      ...
      if (util_is_try_error(..v)) {
        util_error(
          c(
            "Internal error in util_finalize_sizing_hints, parsing final sizing", # nolint: line_length_linter.
            "hints, sorry. Please report: ",
  ...
  # CSS calc examples can use screen-height times a numeric factor in pixels.
  # css does not allow space between number and unit; allowed units according
  # to https://www.w3schools.com/cssref/css_units.php
  # cm, mm, in, px, pt, pc // relative: em, ex, ch, rem, vw, vh, vmin, vmax, %
  # relative, unsupported by the units package: px, pt, pc, em, ex, ch, rem,
  # vw, vh vmin, vmax, %
...
#' Internal helper: util finalize sizing hints
#'
#' @noRd
.util_finalize_sizing_hints <- function(sizing_hints) {
  ## CASE 1: bar_chart
  if (!is.null(sizing_hints$figure_type_id) &&
    ...
    # violin_plot, and rotated_plot that include 2 different plot with the axis
    # inverted
...
util_fix_sizing_hints <- function(dqr, x) {
  x_sizing_hints <- util_attr(x, "sizing_hints", exact = TRUE)
  dqr_sizing_hints <- util_attr(dqr, "sizing_hints", exact = TRUE)
  if (!is.null(x_sizing_hints)) {
    if (!is.null(dqr_sizing_hints)) {
      if (!identical(
        x_sizing_hints,
        dqr_sizing_hints
      )) {
        util_error(c(
          "Internal error: two different",
          "sizing_hints should not be,",
          "sorry. Please report this error."
        ))
      }
    }
    attr(dqr, "sizing_hints") <- x_sizing_hints
