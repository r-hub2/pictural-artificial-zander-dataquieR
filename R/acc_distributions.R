...
#' - Selected response variables may include multiple variables, but each
#' variable must have a defined scale level, at least two distinct non-missing
#' values, and must not contain only missing values.
...
acc_distributions <- function(resp_vars = NULL,
  ...
  if (!any(grepl("_RANGE", colnames(meta_data)))) {
    util_message(
      paste(
        "Metadata does not contain columns for expected ranges",
    ...
    if (dq_param == "any") {
      ...
      scale_level <- meta_data[[SCALE_LEVEL]][match(
        resp_vars,
        meta_data[[label_col]]
      )]
      ...
      if (any(!(scale_level %in% c("interval", "ratio")))) {
        ...
        if (exists("res_merged")) {
          ...
          res_merged$SummaryPlotList <- c(
            res_merged$SummaryPlotList,
      ...
      rvs_meta <- util_prep_location_check(
        resp_vars = resp_vars,
        meta_data = meta_data,
      ...
      rvs_meta <- util_prep_proportion_check(
        resp_vars = resp_vars,
        meta_data = meta_data,
        ds1 = ds1,
        report_problems = "warning",
  ...
  if (dq_param == "location") {
    dq_check_list <- lapply(setNames(nm = resp_vars), function(rv) {
      ...
      loc_res <- setNames(
        nm = c(
          "Variables",
          # Legacy location flag and value names are kept explicit here.
    ...
    dq_check_list <- lapply(setNames(nm = resp_vars), function(rv) {
      ...
      prop_table <- prop_table[union(
        names(rvs_meta$Range[[rv]]),
        names(prop_table)
      )]
      ...
      prop_res <- setNames(
        nm = c(
          "Variables",
          # Legacy proportion flag and value names are kept explicit here.
    ...
    flg_cat_prop <- lapply(dq_check_list, function(x) {
      x[["flg_which"]]
    })
  ...
  # summary table --------------------------------------------------------------
  if (ncol(res_dq_check) == 0) { # in case there is no check for location or range, it provides a table without columns # nolint: line_length_linter.
    res_out <- res_dq_check
    res_view <- res_dq_check
  } else {
    res_out <- res_dq_check[, 1:4, drop = FALSE]
    # GRADING for backwards compatibility
    res_out$GRADING <- as.numeric(res_out[, 2, drop = TRUE])
  ...
  if (dq_param == "location") {
    res_view <- data.frame(
      "Variables" = res_dq_check$Variables,
      "Measure of location" = res_dq_check$loc_func,
      "Value" = res_dq_check$values_from_data,
      "Range of expected values" =
        res_dq_check$loc_range,
      "GRADING" = as.numeric(res_dq_check$FLG_acc_ud_loc),
      check.names = FALSE
    )
    # Add new attribute to the columns of the table to
    # define the datatype of each column
    attr(res_view$Variables, DATA_TYPE) <- DATA_TYPES$STRING
    attr(res_view$"Measure of location", DATA_TYPE) <-
      DATA_TYPES$STRING
    attr(res_view$Value, DATA_TYPE) <-
      DATA_TYPES$FLOAT
    attr(res_view$"Range of expected values", DATA_TYPE) <-
      DATA_TYPES$STRING
    attr(res_view$GRADING, DATA_TYPE) <- DATA_TYPES$INTEGER
  } else if (dq_param == "proportion") {
    res_view <- data.frame(
      "Variables" = res_dq_check$Variables,
      ...
      "GRADING" = as.numeric(res_dq_check$FLG_acc_ud_prop),
      check.names = FALSE
    )
    # Add new attribute to the columns of the table to
    # define the datatype of each column
    attr(res_view$Variables, DATA_TYPE) <- DATA_TYPES$STRING
    attr(res_view$Proportions, DATA_TYPE) <-
      DATA_TYPES$STRING
    attr(res_view$"Range of expected values", DATA_TYPE) <-
      DATA_TYPES$STRING
    attr(res_view$GRADING, DATA_TYPE) <- DATA_TYPES$INTEGER
  ...
  plot_list <- lapply(setNames(nm = resp_vars), function(rv) {
    ...
    if (plot_histogram) {
      ...
      if (plot_ranges && any(!is.na(flg_cat_prop[[rv]]))) {
        ...
        plot_data[plot_data[["cat"]] %in% flg_cat_prop[[rv]], "col"] <- blue_red[2] # nolint: line_length_linter.
      }
      ...
      p <- util_bar_plot(
        ...
        num_var = "count",
        fill_var = "col",
        colors = unique(plot_data[, "col", drop = TRUE]),
      ...
      if (dq_param == "proportion" && rv %in% rvs_with_meta) {
        ...
        segments_list <- lapply(
          ...
          function(pp) {
            ...
            if (!inherits(interval_pp, "interval")) {
              return(NULL)
            }
    ...
    # Historical bar-chart sizing hint prototype removed here. Inspect commit
    # e7c6dc20b1 before restoring layer-based sizing attributes.
  ...
  if (length(resp_vars) == 1) {
    ...
    if (!is.null(plot_list[[resp_vars]])) {
      ...
      # in case of ecdf
      if (length(util_gg_get(obj1, "layout")$facet_params$.possible_columns) == 2) { # nolint: line_length_linter.
        min_bar_height <- 0
        max_bar_height <- 0
      ...
      if (inherits(ds1[[resp_vars]], "POSIXct")) {
        ...
        # Historical bar-count special cases removed here. Inspect commit
        # e7c6dc20b1 before restoring check-specific bar counts.
...
acc_distributions_only <- function(resp_vars = NULL,
  ...
  prep_prepare_dataframes(
    .replace_hard_limits = TRUE,
    .apply_factor_metadata = TRUE, # can be omitted in favor of .apply_factor_metadata_inadm # nolint: line_length_linter.
    .apply_factor_metadata_inadm = TRUE
  )
  ...
  if (.called_in_pipeline && scl_lvl %in% c(SCALE_LEVELS$NOMINAL, SCALE_LEVELS$ORDINAL)) { # nolint: line_length_linter.
...
util_as_plotly_acc_distributions <- function(res, ...) {
    ...
    return(util_ggplotly(ggplot2::ggplot() +
      ggplot2::annotate("text",
        ...
        label =
          sprintf(
            paste(
              "Internal error: I should  have exactly 1 result, if",
  ...
  withCallingHandlers(
    {
      py <- util_ggplotly(res$SummaryPlot, ...) # xxxx remove withCallingHandlers after removing xxxxx from util_histogram # nolint: line_length_linter.
    },
    warning = function(w) {
