...
.get_internal_api <- function(fkt, version = API_VERSION, or_newer = TRUE) {
  # more life-cycle stuff, also for deprecation
  version_was_missing <- missing(version)
  version <- try(as.package_version(version), silent = TRUE)
  if (util_is_try_error(version)) {
    util_error(
      "You need to pass a version number in argument %s",
      sQuote("version")
    )
  }
