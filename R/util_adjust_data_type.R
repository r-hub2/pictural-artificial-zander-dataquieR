  ...
  if (!isTRUE(util_attr(study_data, "Data_type_matches", exact = TRUE))) {
    ...
    if (any(check_type != 1)) {
      ...
      transf_sd <- lapply(
        ...
        function(rv) {
          rv2 <- util_data_type_conversion(
            study_data[[rv]],
            expected_type[[rv]]
          )
          if (!identical(
            which(is.na(study_data[[rv]])),
            which(is.na(rv2))
          )) {
            if (length(relevant_vars_for_warnings) == 0 || # did not find relevant vars vector, show warning in any case # nolint: line_length_linter.
    ...
    logcs <- names(which(vapply(study_data, is.logical, FUN.VALUE = logical(1)))) # these should be NA only. # nolint: line_length_linter.
    if (identical(Sys.getenv("TESTTHAT"), "true")) {
      util_stop_if_not(
        "all logical columns should be NA only." =
          all(is.na(unlist(as.vector(study_data[, logcs, drop = TRUE]))))
      )
    }
    if (length(logcs) > 0) {
      expected_type <- setNames(
        util_map_labels( # convert them to the desired NA type -- can happen, if data type was compatible, so no conversion was done # nolint: line_length_linter.
          x = logcs,
          to = DATA_TYPE,
          from = VAR_NAMES,
          ifnotfound =
      ...
      study_data[, lgcs_which_are_intgs] <-
        lapply(
          study_data[, lgcs_which_are_intgs, drop = FALSE],
          as.integer
        )
      study_data[, lgcs_which_are_flts] <-
        lapply(
          study_data[, lgcs_which_are_flts, drop = FALSE],
          as.double
        )
    }
