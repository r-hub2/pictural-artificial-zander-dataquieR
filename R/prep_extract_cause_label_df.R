...
prep_extract_cause_label_df <- function(item_level = "item_level",
      ...
      FUN =
        function(x, v, tp) {
          if (!is.na(tp) && tp == DATA_TYPES$DATETIME) {
            ...
            prfx[is.na(prfx)] <- FALSE
            if (any(prfx, na.rm = TRUE)) {
              dfr[prfx, CODE_LABEL] <-
                paste(cls, dfr[prfx, CODE_LABEL, drop = TRUE])
            }
            dfr$CODE_CLASS <- rep(cls, nrow(dfr))
            dfr$resp_vars <- rep(v, nrow(dfr))
            dfr[!is.na(dfr$CODE_VALUE), , drop = FALSE]
  ...
  meta_data[[MISSING_LIST]][is.na(meta_data[[MISSING_LIST]]) & ml_empty_on_purpose] <- # nolint: line_length_linter.
    SPLIT_CHAR
