...
prep_get_study_data_segment <- function(
  ...
  if (util_is_try_error(meta_data_segment)) {
    util_warning(
      "%s metadata missing/corrupted: %s",
      sQuote("segment_level"),
      conditionMessage(util_attr(meta_data_segment, "condition",
          exact = TRUE
        ))
    )
  ...
  util_ensure_in(
    applicability_problem = TRUE, error = TRUE,
    segment, meta_data[[STUDY_SEGMENT]],
    sprintf(
      "Did not find segment %s in item_level metadata %s",
  ...
  res <- util_remove_empty_rows(
    id_vars = id_vars,
    ds1[, unique(c(id_vars, util_get_vars_in_segment(segment,
            meta_data = meta_data,
            label_col = label_col
          ))), drop = FALSE]
  )
