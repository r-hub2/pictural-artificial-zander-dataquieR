...
prep_add_cause_label_df <- function(item_level = "item_level",
  ...
  amend_cl <- function(rv, lst, tp) {
    ...
    if (lst_nm %in% colnames(meta_data)) {
      cl <- meta_data[meta_data[[label_col]] == rv, lst_nm, drop = TRUE]
    } else {
      cl <- NA_character_
    }
