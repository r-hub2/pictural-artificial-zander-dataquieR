...
con_limit_deviations <- function(resp_vars = NULL,
  ...
  known_limits <- unlist(
    WELL_KNOWN_META_VARIABLE_NAMES[intersect(
      ...
      which(vapply(WELL_KNOWN_META_VARIABLE_NAMES,
        function(ll) {
          util_attr(ll, "var_att_required", exact = TRUE) != "technical"
        },
        FUN.VALUE = logical(1)
      ))
    )]
  ...
  if (!DATA_PREPARATION %in% colnames(meta_data_cross_item)) {
    meta_data_cross_item[[DATA_PREPARATION]] <-
      # never use LIMITS here, we are assessing the limits
      rep("LABEL | MISSING_NA", nrow(meta_data_cross_item))
  } else {
    meta_data_cross_item[[DATA_PREPARATION]] <-
      gsub(
        ignore.case = TRUE,
  ...
  if (!all(util_empty(as.vector(meta_data_cross_item[
    ...
    all_viols <- lapply(given_cross_limits, function(lim) {
      # reduce the cross-item_level content to rows that contain limits
      cur_cross <-
      ...
      if (nrow(cur_cross) == 0) {
        ...
        # overwrite contraction_term with the rule of the limit
        ...
        cur_cross <- cur_cross[which_rows, , drop = FALSE]
        if (nrow(cur_cross) == 0) {
          return(NULL)
        } else {
          ...
          r <- suppressWarnings(con_contradictions_redcap(
            ...
            meta_data = meta_data,
            label_col = label_col,
            meta_data_cross_item = cur_cross
          )$VariableGroupTable)
          # merge by check_id
          r[[VARIABLE_LIST]] <-
            merge(cur_cross, r,
              by = CHECK_ID, all = FALSE,
              suffixes = c("", ".y")
            )[[VARIABLE_LIST]]
          dt <- util_map_labels(
            r[[VARIABLE_LIST]],
          ...
          util_con_limit_deviations_cross_item_metrics(r, lim)
        }
      }
    })
    all_viols <- util_rbind(data_frames_list = all_viols)
    # obtain a data frame with all the results for limits from the cross-item
    # level
  ...
  if (!is.null(limits) && !(all(limits %in% colnames(meta_data)))) {
    ...
    util_warning(
      paste0(
        "The limit deviation check cannot be performed for ",
  ...
  if (length(LIMITS) > 0) {
    empty_limit_column <- apply(
      meta_data[, LIMITS, drop = FALSE], 2,
      function(cc) {
        all(util_empty(cc))
      }
    )
  ...
  if (length(LIMITS) == 0) {
    util_message(paste0(
      "Hint: If your metadata contains a column with limits ",
    ...
    util_error(
      paste0(
        "The limit deviation check cannot be performed without ",
  ...
  if (length(resp_vars) == 0) {
    ...
    if (all(!any_limits)) {
      ...
      resp_vars_iteml <-
        nmrvl[util_map_labels(nmrvl,
          meta_data = meta_data, to = DATA_TYPE,
      ...
      resp_vars_iteml <- intersect(resp_vars_iteml, resp_vars)
  ...
  # Resp_vars----
  # resp_vars now contains variables with limits in item level or cross item
  # level
  ...
  if (exists("resp_vars_iteml") && length(resp_vars_iteml) > 0) {
    ...
    md_lim_int <- setNames(
      ...
      lapply(resp_vars_iteml, function(rvs) {
        ...
        lim_int <- lapply(lim, util_parse_stratified_limit)
        is_interval <- vapply(lim_int,
          FUN.VALUE = logical(1),
          function(int) {
            inherits(int, "interval") || inherits(int, "stratified_limit")
          }
        )
        ...
        if (all(!is_interval)) {
          ...
          if (return_limit_categorical == FALSE) {
            var_scale_lev <- meta_data[
              meta_data[[label_col]] == rvs,
              SCALE_LEVEL,
              drop = FALSE
            ]
            is_categorical <- var_scale_lev %in% c(
    ...
    stratified_limit_data_preparation <- setNames(
      nm = resp_vars_iteml,
      vapply(resp_vars_iteml, function(rv) {
        util_stratified_limit_data_preparation(
          meta_data = meta_data,
          label_col = label_col,
          rv = rv
        )
    ...
    md_lim_all_segments <- setNames(
      ...
      lapply(resp_vars_iteml, function(rv) {
        lapply(md_lim_int[[rv]], function(int) {
          if (inherits(int, "stratified_limit")) {
            return(NULL)
          }
          ll <- int
          ll$upp <- int$low
    ...
    sd_lim <- setNames(
      nm = resp_vars_iteml,
      lapply(resp_vars_iteml, function(rvs) {
        col_rv <- ds1[, rvs, drop = TRUE]
        lim <- md_lim_int[[rvs]]
        col_int <- lapply(lim, function(int) {
          if (inherits(int, "stratified_limit")) {
            return(util_classify_stratified_limits(
              col_rv,
              util_resolve_stratified_limit(
                int,
                ds1 = ds1,
                meta_data = meta_data,
                data_preparation = stratified_limit_data_preparation[[rvs]]
              )
            ))
          }
          col_out <- rep(NA, length(col_rv))
          if (int$inc_l) below <- col_rv < int$low else below <- col_rv <= int$low # nolint: line_length_linter.
          within <- redcap_env$`in`(col_rv, int)
          if (int$inc_u) above <- col_rv > int$upp else above <- col_rv >= int$upp # nolint: line_length_linter.
    ...
    sd_lim_hl_check <- setNames(
      ...
      lapply(resp_vars_iteml, function(rvs) {
        ...
        if ("HARD_LIMITS" %in% names(sd_lim_rvs) && length(sd_lim_rvs) > 1) {
          hl_viol_ind <- which(
            sd_lim_rvs[[HARD_LIMITS]] %in% c("below", "above")
          )
      ...
      spec_lines <- rbind(
        ...
        data.frame(
          ...
          color = rep(gray.colors(n = ceiling(n_spec_lim / length(base_lty))),
            each = length(base_lty)
          )[1:n_spec_lim],
          lty = c(
            rep(base_lty, floor(n_spec_lim / length(base_lty))),
    ...
    plot_list <- lapply(setNames(nm = resp_vars_iteml), function(rv) {
      if (isTRUE(has_stratified_limits[[rv]])) {
        return(
          util_create_stratified_limit_plot(
            rv = rv,
            ds1 = ds1,
            meta_data = meta_data,
            label_col = label_col,
            limits = md_lim_int[[rv]],
            limit_results = sd_lim_hl_check[[rv]],
            data_preparation = stratified_limit_data_preparation[[rv]],
            is_datetime_var = is_datetime_var[[rv]],
            is_time_var = is_time_var[[rv]],
            spec_txt = spec_txt,
            ref_env = ref_env,
            show_obs = show_obs
          )
      ...
      rv_data <- rv_data[complete.cases(rv_data[, "values", drop = TRUE]), , drop = FALSE] # nolint: line_length_linter.
      ...
      # Füllungen
      rv_data$viol_n_limits <- apply(
        rv_data[, rv_limit_names, drop = FALSE], 1,
      ...
      spec_bars <- unique(rv_data[
        ,
        c(rv_limit_names, "limit_violations", "fill")
        , drop = TRUE])
      ...
      if (plot_histogram) {
        ...
        if (length(plot_segments_intervals) != length(bin_breaks)) {
          bin_segments_with_data <-
            vapply(bin_breaks, FUN.VALUE = logical(1), function(bb) {
              sum(
                vapply(plot_segments_intervals,
                  FUN.VALUE = logical(1),
                  function(p_int) {
        ...
        if (is_datetime_var[rv]) {
          plot_data[["histogram_x"]] <- util_parse_date(
            plot_data[["histogram_x"]]
          )
        }
    ...
    heatmap_tab <- do.call(
      rbind.data.frame,
      lapply(setNames(nm = resp_vars_iteml), function(rv) {
        n_viol <- rowSums(freq_lim_viol[[rv]][
          ,
          c("below", "above"),
          drop = FALSE
        ])
        ...
        out_viol <- setNames(
          rep(NA, length(report_all_limits)),
          report_all_limits
        )
    ...
    heatmap_tab <- util_new_report_summary_table(heatmap_tab,
      meta_data = meta_data,
      label_col = label_col
    )
    ...
    sumtab_inadm_num <- vapply(
      resp_vars_iteml,
      FUN.VALUE = numeric(2), FUN = function(rv) {
        if (is_datetime_var[[rv]]) {
          res <- c(NA, nrow(ds1))
        } else {
          sumdat_sub <- sumdat %>%
      ...
      FUN.VALUE = numeric(2), FUN = function(rv) {
        if (is_datetime_var[[rv]]) {
          ...
          if (length(rv_sd_int) == 0) {
            ...
            res <- c(
              length(which(rv_sd_int$N_lim > 0)),
              nrow(rv_sd_int)
            )
          }
        }
    ...
    sumtab <- data.frame(
      ...
      "NUM_con_rvv_unum" = sumtab_unc_num["n_viol", , drop = TRUE],
      "PCT_con_rvv_unum" = round(sumtab_unc_num["n_viol", , drop = TRUE] /
          sumtab_unc_num["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_unum" = ifelse(sumtab_unc_num["n_viol", , drop = TRUE] > 0, TRUE, FALSE), # nolint: line_length_linter.
      "NUM_con_rvv_utdat" = sumtab_unc_datetime["n_viol", , drop = TRUE],
      "PCT_con_rvv_utdat" = round(sumtab_unc_datetime["n_viol", , drop = TRUE] / # nolint: line_length_linter.
          sumtab_unc_datetime["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_utdat" = ifelse(sumtab_unc_datetime["n_viol", , drop = TRUE] > 0, # nolint: line_length_linter.
        TRUE, FALSE
      ),
      "NUM_con_rvv_inum" = sumtab_inadm_num["n_viol", , drop = TRUE],
      "PCT_con_rvv_inum" = round(sumtab_inadm_num["n_viol", , drop = TRUE] /
          sumtab_inadm_num["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_inum" = ifelse(sumtab_inadm_num["n_viol", , drop = TRUE] > 0, # nolint: line_length_linter.
        TRUE, FALSE
      ),
      "NUM_con_rvv_itdat" = sumtab_inadm_datetime["n_viol", , drop = TRUE],
      "PCT_con_rvv_itdat" = round(sumtab_inadm_datetime["n_viol", , drop = TRUE] / # nolint: line_length_linter.
          sumtab_inadm_datetime["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_itdat" = ifelse(sumtab_inadm_datetime["n_viol", , drop = TRUE] > 0, # nolint: line_length_linter.
        TRUE, FALSE
      )
    )
    sumtab$PCT_con_rvv_unum[which(sumtab_unc_num["n", , drop = TRUE] == 0)] <- 0 # nolint: line_length_linter.
    sumtab$PCT_con_rvv_utdat[which(sumtab_unc_datetime["n", , drop = TRUE] == 0)] <- 0 # nolint: line_length_linter.
    sumtab$PCT_con_rvv_inum[which(sumtab_inadm_num["n", , drop = TRUE] == 0)] <- 0 # nolint: line_length_linter.
    sumtab$PCT_con_rvv_itdat[which(sumtab_inadm_datetime["n", , drop = TRUE] == 0)] <- 0 # nolint: line_length_linter.
    ...
    sumdat_wide$"All.outside.limitsN" <-
      sumdat_wide$Number.below + sumdat_wide$Number.above
    sumdat_wide$"All.outside.limits%" <- round(
      sumdat_wide$"All.outside.limitsN" /
        sumdat_wide$Number.samplesize * 100,
      digits = 2
    )
    sumdat_wide$`Below.limits-N (%)` <- paste0(
      sumdat_wide$Number.below,
      " (",
      sumdat_wide$Percentage.below,
      ")"
    )
    sumdat_wide$`Within.limits-N (%)` <- paste0(
      sumdat_wide$Number.within,
      " (",
      sumdat_wide$Percentage.within,
      ")"
    )
    sumdat_wide$`Above.limits-N (%)` <- paste0(
      sumdat_wide$Number.above,
      " (",
      sumdat_wide$Percentage.above,
      ")"
    )
    sumdat_wide$`All outside limits N (%)` <- paste0(
      sumdat_wide$All.outside.limitsN, " (",
      sumdat_wide$`All.outside.limits%`, ")"
    )
    sumdat_wide <- sumdat_wide[, c(
      "Variables", "Limits", "Below.limits-N (%)",
      "Within.limits-N (%)", "Above.limits-N (%)",
      "All outside limits N (%)"
    ), drop = FALSE]
    colnames(sumdat_wide) <- c(
      "Variables", "Limits", "Below limits N (%)",
      "Within limits N (%)", "Above limits N (%)",
      "All outside limits N (%)"
    )
  ...
  if (!is.null(vars_in_cil)) {
    ...
    cil_limits <- setNames(
      ...
      lapply(vars_in_cil, function(rvs) {
        ...
        r_complex_var <- r_complex[r_complex$Variables == rvs, , drop = FALSE]
        ...
        col_int <- lapply(lims, function(ll) {
          ...
          r_complex_limit <- r_complex_var[r_complex_var$limits == ll, , drop = FALSE] # nolint: line_length_linter.
          ...
          col_to_use <- num_cols[!is.na(r_complex_limit[, num_cols, drop = TRUE])] # nolint: line_length_linter.
          # Extract the selected value if a column was found.
          if (length(col_to_use) == 1) {
            all_outliers <- r_complex_limit[[col_to_use]]
          } else {
            all_outliers <- NA
          }
          n_tot <- length(ds1[, rvs, drop = TRUE]) # sample size from ds1
      ...
      do.call(
        ...
        lapply(
          ...
          function(rv) {
            ...
            r_complex_var <- r_complex[r_complex$Variables == rv, , drop = FALSE] # nolint: line_length_linter.
            # Extract the limits for the current variable
            lims <- r_complex_var$limits
    ...
    heatmap_tab_complex <- do.call(
      rbind.data.frame,
      lapply(
        setNames(nm = vars_in_cil),
        function(rv) {
          cur_row <- r_complex[r_complex$Variables == rv &
              r_complex$limits == "HARD_LIMITS", , drop = FALSE]
          if (nrow(cur_row) > 0) {
            if (dt_rcomplex[cur_row[
              cur_row$Variables == rv,
              "Variables"
              , drop = TRUE]] != "datetime") {
              n_viol <- cur_row[
                cur_row$Variables == rv,
            ...
            divisor <- cur_row[cur_row$Variables == rv, "N", drop = TRUE]
            n_viol_rel <- ifelse(divisor == 0, 0, n_viol / divisor)
            names(n_viol_rel) <- "HARD_LIMITS"
          }
          cur_row <- r_complex[r_complex$Variables == rv &
              r_complex$limits == "SOFT_LIMITS", , drop = FALSE]
          if (nrow(cur_row) > 0) {
            if (dt_rcomplex[cur_row[
              cur_row$Variables == rv,
              "Variables"
          ...
          cur_row <- r_complex[r_complex$Variables == rv &
              r_complex$limits == "DETECTION_LIMITS", , drop = FALSE]
          if (nrow(cur_row) > 0) {
            if (dt_rcomplex[cur_row[
              cur_row$Variables == rv,
              "Variables"
    ...
    heatmap_tab_complex <- util_new_report_summary_table(heatmap_tab_complex,
      meta_data = meta_data,
      label_col = label_col
    )
    ...
    r_complex$dt <- dt_rcomplex[match(r_complex$Variables, names(dt_rcomplex))]
      ...
      FUN.VALUE = numeric(2), FUN = function(rv) {
        if (is_datetime_var_cil[[rv]]) {
          ...
          sumdat_sub <- sumdat_complex[sumdat_complex$Variables == rv &
              ...
              sumdat_complex$Section == "all_outliers", , drop = FALSE]
          if (length(sumdat_sub[, "Number", drop = TRUE]) > 0) {
            n_viol <- sumdat_sub[, "Number", drop = TRUE]
          } else {
            n_viol <- NA
          }
          n_tot <- nrow(ds1)
    ...
    sumtab_complex <- data.frame(
      ...
      "NUM_con_rvv_unum" = sumtab_unc_num_complex["n_viol", , drop = TRUE],
      "PCT_con_rvv_unum" = round(sumtab_unc_num_complex["n_viol", , drop = TRUE] / # nolint: line_length_linter.
          sumtab_unc_num_complex["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_unum" = ifelse(
        sumtab_unc_num_complex["n_viol", , drop = TRUE] > 0,
        TRUE,
        FALSE
      ),
      "NUM_con_rvv_utdat" = sumtab_unc_datetime_complex["n_viol", , drop = TRUE], # nolint: line_length_linter.
      "PCT_con_rvv_utdat" = round(sumtab_unc_datetime_complex["n_viol", , drop = TRUE] / # nolint: line_length_linter.
          sumtab_unc_datetime_complex["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_utdat" = ifelse(sumtab_unc_datetime_complex["n_viol", , drop = TRUE] > 0, # nolint: line_length_linter.
        TRUE, FALSE
      ),
      "NUM_con_rvv_inum" = sumtab_inadm_num_complex["n_viol", , drop = TRUE],
      "PCT_con_rvv_inum" = round(sumtab_inadm_num_complex["n_viol", , drop = TRUE] / # nolint: line_length_linter.
          sumtab_inadm_num_complex["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_inum" = ifelse(
        sumtab_inadm_num_complex["n_viol", , drop = TRUE] > 0,
        TRUE,
        FALSE
      ),
      "NUM_con_rvv_itdat" = sumtab_inadm_datetime_complex["n_viol", , drop = TRUE], # nolint: line_length_linter.
      "PCT_con_rvv_itdat" = round(sumtab_inadm_datetime_complex["n_viol", , drop = TRUE] / # nolint: line_length_linter.
          sumtab_inadm_datetime_complex["n", , drop = TRUE] * 100, 2),
      "FLG_con_rvv_itdat" = ifelse(
        sumtab_inadm_datetime_complex["n_viol", , drop = TRUE] > 0,
        TRUE,
        FALSE
      ),
      check.names = FALSE, fix.empty.names = FALSE
    ...
    sumtab_complex$PCT_con_rvv_unum[
      which(sumtab_unc_num_complex["n", , drop = TRUE] == 0)
    ] <- 0
    sumtab_complex$PCT_con_rvv_utdat[
      which(sumtab_unc_datetime_complex["n", , drop = TRUE] == 0)
    ] <- 0
    sumtab_complex$PCT_con_rvv_inum[
      which(sumtab_inadm_num_complex["n", , drop = TRUE] == 0)
    ] <- 0
    sumtab_complex$PCT_con_rvv_itdat[
      which(sumtab_inadm_datetime_complex["n", , drop = TRUE] == 0)
    ] <- 0
    ...
    plot_list_complex <- lapply(setNames(nm = vars_in_cil), function(rv) {
      r_complex_rv <- r_complex[r_complex$Variables == rv, , drop = FALSE]
      ...
      plot_data <- lapply(setNames(nm = r_complex_rv$limits), function(lim) {
        r_complex_rv_lim <- r_complex_rv[r_complex_rv$limits == lim, , drop = FALSE] # nolint: line_length_linter.
        PCT_content <- r_complex_rv_lim[
          ,
          grep("^PCT_", names(r_complex_rv_lim))
          , drop = FALSE]
        PCT_content <- PCT_content[
          ,
          sapply(PCT_content, function(x) !all(is.na(x)))
          , drop = FALSE]
        if (length(PCT_content) != 1) {
          util_error(
            m = "There should be only one value for the number of violations per limit" # nolint: line_length_linter.
          )
        }
        PCT_content[[1]]
      })
      ...
      if (max_bin_height == min_bin_height) {
        range_height <- max_bin_height
      } else {
        range_height <- max_bin_height - min_bin_height
      }
  ...
  if (!exists("sumtab_complex")) {
    sumtab_complex <- NULL
  }
  if (!exists("sumdat_wide")) {
    sumdat_wide <- NULL
  }
  if (!exists("sumdat_wide_complex")) {
    sumdat_wide_complex <- NULL
  }
  if (!exists("heatmap_tab")) {
    heatmap_tab <- NULL
  }
  if (!exists("heatmap_tab_complex")) {
    heatmap_tab_complex <- NULL
  }
  if (!exists("plot_list")) {
    plot_list <- NULL
  }
  ...
  for (cl in c(
    ...
    attr(sumdat_wide_final[[cl]], DATA_TYPE) <- DATA_TYPE_NUMBER_PAREN
  }
  ...
  # History: this formerly hard-coded a result layout; use git blame/log around
  # this return block if the old layout code is needed for comparison.
...
#' Map cross-item contradiction metrics to limit-deviation metrics
#'
#' @noRd
util_con_limit_deviations_cross_item_metrics <- function(r, lim) {
  # Add a temporary data-type column to split cross-item contradiction results
  # into numeric and datetime limit-deviation metrics.
  data_type <- r$data_type
  data_type[util_empty(data_type)] <- NA_character_
  ...
  unexpected_data_types <- unique(data_type[
    is.na(data_type) | !(data_type %in% expected_data_types)
  ])
  if (length(unexpected_data_types) > 0) {
    # Cross-item limits are specified via the metadata table and should already
    # be restricted to numeric or datetime-like variables by applicability. If
    # that contract breaks, keep returning the legacy table shape but report it.
    util_warning(
      "Unexpected %s for cross-item limit mapping: %s",
      dQuote(DATA_TYPE),
      util_pretty_vector_string(unexpected_data_types),
      applicability_problem = TRUE
    )
  }
  if (lim == HARD_LIMITS) {
    # HARD_LIMITS use con_con_contc and map to invalid numeric or datetime
    # values in the resulting limit-deviation table.
    r_datetime_hard <- r[
      !is.na(r$data_type) & r$data_type == DATA_TYPES$DATETIME,
      ,
      drop = FALSE
    ]
    ...
    r_numeric_hard <- r_numeric_hard[, colnames(r_datetime_hard), drop = FALSE]
    final_r_hard <- rbind(r_datetime_hard, r_numeric_hard)
    final_r_hard$data_type <- NULL
    final_r_hard$limits <- lim
    return(final_r_hard)
  }

  # Detection and soft limits use con_con_contu and map to unlikely numeric or
  # datetime values in the resulting limit-deviation table.
  r_datetime <- r[
    !is.na(r$data_type) & r$data_type == DATA_TYPES$DATETIME,
    ,
    drop = FALSE
  ...
  r_numeric <- r_numeric[, colnames(r_datetime), drop = FALSE]
  final_r <- rbind(r_datetime, r_numeric)
  final_r$data_type <- NULL
  final_r$limits <- lim
  final_r
}
