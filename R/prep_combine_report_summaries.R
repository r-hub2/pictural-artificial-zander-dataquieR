...
prep_combine_report_summaries <- function(...,
  ...
  if (!all(vapply(summaries_list, function(summary) {
    ...
    util_error(
      c(
        "All summaries passed must be returned by %s or this function",
  ...
  if (any(vapply(lapply(lapply(combined, `[[`, VAR_NAMES), duplicated),
    ...
    util_warning(
      c(
        "Some of the summaries comprise overlapping variables, will",
    ...
    combined <-
      lapply(combined, function(x) {
        x <- x[!duplicated(x[[VAR_NAMES]]), , drop = FALSE]
        x
      })
  }
