...
prep_fix_meta_id_dups <- function(meta_data_segment = "segment_level",
  ...
  all_id_vars <-
    trimws(unique(unlist(util_parse_assignments(
  ...
  duplicated_il_wo <- duplicated(meta_data[, setdiff(
    colnames(meta_data),
    c(
      STUDY_SEGMENT,
      DATAFRAMES
  ...
  if (any(duplicated_vn & !duplicated_il_wo)) {
    ...
    dup_names <- as.vector(meta_data[duplicated_vn, VAR_NAMES, drop = TRUE])
    if (!all(dup_names %in% all_id_vars)) {
      util_warning(
        c(
          "Some none-id-vars are duplicated but differ only in %s or %s",
      ...
      util_message(
        paste0(
          ...
          "into the shared INTRO location."
        ),
        applicability_problem = TRUE
      )
    }
    meta_data[
      meta_data[[VAR_NAMES]] %in% dup_names,
      intersect(
        colnames(meta_data),
        c(
          DATAFRAMES,
      ...
      "**INTRO**"
    r <- meta_data[!duplicated_vn, , drop = FALSE]
    return(r)
  }
}
