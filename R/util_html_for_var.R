...
#'                              (if needed)
#' @param rendered_repsum [data.frame] the `dataquieR` summary, see
#'   [summary()],
...
util_html_for_var <- function(results, cur_var, use_plot_ly, template,
  ...
  function_alias_map) {
  fname <-
    sprintf("VAR_%s.html", prep_link_escape(cur_var, # remove special characters from variable name # nolint: line_length_linter.
        html = TRUE
      ))
  ...
  this <- util_attr(rendered_repsum, "this", exact = TRUE)
  repsum_wide <- util_attr(rendered_repsum, "repsum_wide", exact = TRUE)
  Variables <- util_map_labels(
    repsum_wide$Variables, meta_data, label_col,
    util_attr(repsum_wide, "label_col", exact = TRUE),
    ifnotfound = repsum_wide$Variables
  )
  if (any(Variables == cur_var)) {
    rs <- repsum_wide[
      Variables == cur_var,
      setdiff(colnames(repsum_wide), "Variables"), drop = FALSE
    ]
    rs <- data.frame(x = colnames(rs), v = unname(unlist((rs[1, , drop = TRUE])))) # nolint: line_length_linter.
    rs <- rbind.data.frame(
      rs[rs$x != "Total", , drop = FALSE],
      rs[rs$x == "Total", , drop = FALSE]
    )
    rs <-
      rs[!grepl("background: #ffffff", rs[, 2, drop = TRUE], fixed = TRUE), , drop = FALSE] # nolint: line_length_linter.
    rs <-
      rs[!is.na(rs[, 2, drop = TRUE]), , drop = FALSE]

    # The rendered summary cell carries the DQ grading order in `sort`.
    grading_order <- sub('^.*sort="([0-9]+)".*$', "\\1", rs$v)
    grading_order[grading_order == rs$v] <- NA_character_
    grading_order <- suppressWarnings(as.integer(grading_order))
    rs <- rs[order(
      rs$x == "Total",
      is.na(grading_order),
      grading_order,
    ...
    texts <- vapply(rs[, 1, drop = TRUE],
      util_alias2caption,
      long = TRUE, FUN.VALUE = character(1)
    )
    ...
    # 3) navigateDataquieRFrame(&quot;...&quot;)
    href3 <- sub(
      "^.*navigateDataquieRFrame\\(&quot;([^&]+)&quot;\\).*$",
      "\\1",
      x
    )
    href3[href3 == x] <- NA_character_

    # 4) showDataquieRResult(&quot;...&quot;, &quot;TARGET&quot;,
    # &quot;...&quot;)
    href4 <- sub(
      "^.*showDataquieRResult\\(&quot;[^&]*&quot;\\s*,\\s*&quot;([^&]+)&quot;.*$", # nolint: line_length_linter.
      "\\1",
      x
    )
    href4[href4 == x] <- NA_character_

    # Fallback-Kaskade pro Zeile
    href_final <- ifelse(
      !is.na(href1), href1,
      ifelse(!is.na(href2), href2,
        ifelse(!is.na(href3), href3, href4)
      )
    )
    ...
    links <- vapply(
      mapply(
        href = href_final,
        text = texts,
        no_res = grepl("No results available", rs[, 2, drop = TRUE]),
        SIMPLIFY = FALSE,
        FUN = function(href, text, no_res) {
          if (any(grepl(".", href, fixed = TRUE)) && !no_res) {
            paste0("<a href=\"", href, "\">", text, "</a>")
          } else {
            paste0("<a style=\"color:black;cursor:default;text-decoration:none;\">", text, "</a>") # nolint: line_length_linter.
          }
        }
      ),
      FUN = paste0, collapse = "",
    ...
    # Preserve the DQ grading-category order when the browser table is sorted.
    links <- paste0(
      sprintf('<a sort="%06d" ', seq_along(links)),
      sub("^<a ", "", links)
    )
    links[rs[, 1, drop = TRUE] == "Total"] <- sub(
      'sort="[0-9]+" ',
      'sort="~~~Total" ',
      links[rs[, 1, drop = TRUE] == "Total"],
      perl = TRUE
    )
    rs[, 1] <- links

    # Remove onclick from a elements
    rs[, 2] <- gsub("(<a\\b[^>]*?)\\s+onclick\\s*=\\s*(['\"]).*?\\2", "\\1", rs[, 2, drop = TRUE], perl = TRUE, ignore.case = TRUE) # nolint: line_length_linter.
    rs <- rs[!is.na(href_final), , drop = FALSE]

    if (!!prod(dim(rs))) {
      rep_sum_el <- htmltools::browsable(
        util_html_table(
          rs,
          filter = "top",
  ...
  if (!util_is_empty_html(rep_sum_el)) {
    rep_sum_el <- htmltools::tagList(
      anchor = util_generate_anchor_tag(cur_var, "Summary", "variable"),
      link = util_generate_anchor_link(cur_var, "Summary", "variable"), # these links are moved later # nolint: line_length_linter.
      rep_sum_el
    )
  } else {
    rep_sum_el <- NULL
  ...
  for (cur_dm in dims_in_rep) {
    # Use util_message() locally to trace dimensions during page generation.
    drop_down <- dims[[cur_dm]] # fetches the name of the content for the first-level drop-down menu # nolint: line_length_linter.
    j <- 0
    m <- length(clls_in_rep[[cur_dm]])
    for (cll in clls_in_rep[[cur_dm]]) { # loop over the functions in each dimension # nolint: line_length_linter.
      ...
      # use results as they are, not combined by util_combine_res. see else
      # branch below for comments on mapply
      output <- do.call(htmltools::tagList, unname(mapply(
        dqr = all_of_f,
        nm =
          names(all_of_f),
        FUN =
      ...
      # Use browser() locally to inspect one function/variable result.
      # if we have some result, then store it in the svp list for the current
      # single variable page
      if (length(output) && !all(vapply(output, is.null, FUN.VALUE = logical(1)))) { # nolint: line_length_linter.
        # Historical manual-title fallback removed here.

        title <- util_alias2caption(cll, long = TRUE)
        description <- util_function_description(fkt)
        online_refs <- util_online_ref_candidates(fkt)

        anchor <- output[[1]]$anchor # extract anchor tag and move it above the caption of the current result # nolint: line_length_linter.
        output[[1]]$anchor <- htmltools::span() # delete anchor tag from its orignal position # nolint: line_length_linter.

        svp[[cll]] <- htmltools::tagList(
          htmltools::span( # span is requird to make [[2]][[1]]$link on page navi menu creation below work # nolint: line_length_linter.
            anchor, # put the anchor tag on top
            htmltools::h4(
              style = "margin-top: 3em", # add function name as a ~subtitle, add space to prevent plot.lys to overlap # nolint: line_length_linter.
              htmltools::a(
                target = "_blank",
                class = "dq-online-ref",
                href = online_refs$concept,
                `data-dq-online-ref-href` = online_refs$concept,
                `data-dq-online-ref-fallback-href` =
                  online_refs$implementation,
                `data-dq-online-ref-state` = "unchecked",
                title
              )
            ),
            htmltools::HTML("<!-- "),
  ...
  cur_md <- meta_data[
    meta_data[[label_col]] == cur_var, ,
    drop = FALSE
  ] # subset the metadata for the current variable to show it in the report
  ...
  { # link tables to their representation in this report-site
    ...
    for (cn in grep("_TABLE$", colnames(cur_md), value = TRUE)) {
      cur_md[!util_empty(cur_md[[cn]]), cn] <- vapply(
        FUN.VALUE = character(1),
        cur_md[!util_empty(cur_md[[cn]]), cn, drop = TRUE],
        FUN = function(tn) {
          paste(
  ...
  md_info <- htmltools::tagList( ## link tables
    ...
    link = util_generate_anchor_link(cur_var, "meta_data", "variable"), # these links are moved later # nolint: line_length_linter.
  ...
  if (!is.null(rep_sum_el)) {
    svp <- c(
      list(rep_sum_el = list("", list(rep_sum_el))), # add the rep_sum_el info to the svp keeping the same structure of svp # nolint: line_length_linter.
      svp
    )
  }
  svp <- c(
    svp,
    list(md_info = list("", list(md_info))) # add the metadata info to the svp keeping the same structure of svp # nolint: line_length_linter.
  )
  ...
  is_var_page_js <- htmltools::tags$script(
