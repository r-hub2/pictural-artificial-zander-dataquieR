...
#'     check_type = is.integer, convert_if_possible =
#'       as.integer
#'   )
#' }
#' g(42L)
...
util_expect_scalar <- function(arg_name,
  ...
  if (inherits(arg_value, "try-error")) {
    if (missing_in_parent) {
      ...
      util_warning(
        c(
          "Could not get value of argument %s for unexpected reasons. Setting",
          "to NULL."
        ), arg_name,
        applicability_problem = TRUE
      )
  ...
  if (length(min_length) != 1 || !is.numeric(min_length)) {
    util_error(
      c(
        "Need numeric min_length names in argument %s",
        "wrong use of util_expect_scalar?"
      ),
      arg_name,
  ...
  if (length(max_length) != 1 || !is.numeric(max_length)) {
    util_error(
      c(
        "Need numeric max_length names in argument %s",
        "wrong use of util_expect_scalar?"
      ),
      arg_name,
      applicability_problem = FALSE
  ...
  my_check_type <- function(arg_value) {
    ...
    if (length(type_match) != 1 ||
      ...
      util_error(
        c(
          "Need a lambda function as a predicate as %s for the",
  ...
  if (length(arg_value) > 0 && (!missing(check_type))) {
    ...
    if (!type_match) {
      if (!missing(error_message)) {
        ...
        if (!is.null(error_msg <- util_attr(check_type, "error_msg",
              exact = TRUE
            ))) {
          msg <- error_msg
  ...
  if (!dont_assign) {
    assign(arg_name, arg_value, envir = p)
  } # re-assign the possibly modified
