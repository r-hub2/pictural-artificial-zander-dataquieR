...
util_referred_vars <- function(resp_vars,
  ...
  repeat {
    ...
    referred_vars <-
      unique(unlist(meta_data[meta_data[[VAR_NAMES]] %in%
              vars, key_cols, drop = TRUE], recursive = TRUE))
    ...
    # referred variables from cross-item level metadata from *_LIMITS
    # columns----
    ...
    if (any(sapply(limit_cols, function(col) {
      ...
      variable_list_limits <- lapply(
        X = variable_list_limits,
  ...
  if (length(vars_in_subgroup) != 0) {
    resp_vars_plus_referred <- unique(c(
      resp_vars_plus_referred,
      vars_in_subgroup
    ))
  }
  ...
  if (length(setdiff(resp_vars, temp_subset$VAR_NAMES)) > 0) {
    util_warning(
      "Variables in resp_vars that are not present in the item_level metadata: %s", # nolint: line_length_linter.
  ...
  if (!is.null(meta_data_dataframe) ||
    ...
    if (DATAFRAMES %in% colnames(meta_data) &&
      ...
      # obtain the id from the dataframe interested by the resp-vars and
      # referred vars
      id_vars_referred <- meta_data_dataframe[meta_data_dataframe[[DF_CODE]] %in% # nolint: line_length_linter.
  ...
  if (!is.null(meta_data_segment)) {
    ...
    # obtain the id from the segment interested by the resp-vars and referred
    # vars
    id_vars_seg_referred <- meta_data_segment[meta_data_segment[[STUDY_SEGMENT]] %in% # nolint: line_length_linter.
  ...
  # Create a new complete list of variables with resp_vars, referred_vars and
  # id vars
  ...
  # Create the new item level metadata with only rows of resp vars and
  # referred variables plus id columns.
  rm(temp_subset, vars_referred, key_cols)

  return(list(
    vars_complete = resp_vars_complete,
    md_complete = md_complete
  ))
}
