...
prep_summary_to_classes <- function(report_summary) {
  ...
  if (!inherits(report_summary, "dq_report2_summary") &&
    ...
    util_error(
      c(
        "%s must be returned by %s or this function",
  ...
  meta_columns <- intersect(
    c(VAR_NAMES, STUDY_SEGMENT, colnames(report_summary$meta_data)),
    colnames(report_summary_table)
  )
  metrices_maybe_with_call_name <-
    setNames(nm = setdiff(colnames(report_summary_table), meta_columns))
  ...
  all <- lapply(
    ...
    function(metric_maybe_with_call_name) {
      ...
      col <- setNames(as.numeric(report_summary_table[[metric_maybe_with_call_name]]), # nolint: line_length_linter.
  ...
  rownames(all) <- NULL
  if (!prod(dim(all))) {
    all <- report_summary_table[, c(VAR_NAMES, STUDY_SEGMENT), drop = FALSE]
  } else {
    all <- merge(
      all,
      report_summary_table[, meta_columns, drop = FALSE],
      by = VAR_NAMES,
      all.x = TRUE
    )
    if (have_at_least_some_call_names) {
      ...
      tuple_sizes <- vapply(splitted, length, FUN.VALUE = integer(1)) # should be always 1 or 2 according to function name and result column conventions # nolint: line_length_linter.
      valid_splitted <- tuple_sizes %in% 1:2
      if (!all(valid_splitted)) {
        util_warning(c(
          "For some reason, the result names in this summary",
...
#' Internal helper: metrics to classes
#'
#' @noRd
util_metrics_to_classes <- function(rs_table_long, meta_data, entity = "ITEM") {
  used_metrics <- unique(rs_table_long[["indicator_metric"]])
  used_metrics <- setNames(nm = used_metrics[!is.na(used_metrics)])

  all_thresholds <- lapply(
    used_metrics,
    function(metric) {
      r <- util_get_thresholds(indicator_metric = metric, meta_data = meta_data)
      lapply(r, lapply, util_parse_interval)
  ...
  all <- lapply(
    used_metrics,
    function(metric) {
      curr_metric_rows <- !is.na(rs_table_long$indicator_metric) &
        rs_table_long$indicator_metric == metric
      if (startsWith(metric, "MSG_")) {
        res <- rs_table_long[curr_metric_rows, , drop = FALSE]
        res$class <- res$values_raw
        return(res)
      }
      metric_values <- rs_table_long[
        curr_metric_rows, "values_raw", drop = TRUE
      ]
      true_value_rows <- curr_metric_rows
      true_value_rows[curr_metric_rows] <- metric_values %in% "TRUE"
      false_value_rows <- curr_metric_rows
      false_value_rows[curr_metric_rows] <- metric_values %in% "FALSE"
      rs_table_long[true_value_rows, "values_raw"] <- 1
      rs_table_long[false_value_rows, "values_raw"] <- 0
      col <- suppressWarnings(as.numeric(
        rs_table_long[curr_metric_rows, "values_raw", drop = TRUE]
      ))
      if (entity == "ITEM") {
        names(col) <- paste0(
          rs_table_long[curr_metric_rows, "call_names", drop = TRUE],
          ".",
          rs_table_long[curr_metric_rows, VAR_NAMES, drop = TRUE]
        )
      } else {
        names(col) <- rs_table_long[curr_metric_rows, VAR_NAMES, drop = TRUE]
      }
      thresholds <- all_thresholds[[metric]]
      ...
      classes <- lapply(setNames(nm = names(col)),
        FUN = function(cn_vn) {
          if (entity == "ITEM") {
            vn <- util_sub_string_right_from_.(cn_vn)
          } else {
            vn <- cn_vn
          }
          cl_res <- setNames(mapply(
            ...
            int = thresholds[[vn]],
            FUN = function(clss, int) {
              if (is.na(col[cn_vn]) || !inherits(int, "interval")) {
                return(NA)
              }
              valraw <- col[cn_vn]
          ...
          # Use browser() locally to inspect one acc_distributions threshold.
          return(cl_res)
        }
      )
      # Use browser() locally to inspect all acc_distributions thresholds.
      classes <- lapply(classes, vapply,
        FUN.VALUE = logical(1),
        identity
      )
      classes <- lapply(classes, which)
      if (any(vapply(classes, length, FUN.VALUE = integer(1)) > 1)) {
        util_warning(
          c(
            "Detected invalid grading rule with",
      ...
      res$n_classes <- vapply(
        FUN.VALUE = integer(1),
        rs_table_long[curr_metric_rows, VAR_NAMES, drop = TRUE],
        function(var_name) {
          r <- suppressWarnings(max(as.integer(names(thresholds[[var_name]])),
              na.rm = TRUE
            ))
          if (!is.finite(r)) {
            NA_integer_
  ...
  what_i_need <- unique(all[, setdiff(
    colnames(all),
    c(
      "indicator_metric", "n_classes",
      "class", "value", "values_raw"
    )
  ), drop = FALSE])

  metrics_known_to_be_in_functions2 <- util_rbind(
    data_frames_list =
      lapply(
        names(metrics_known_to_be_in_functions),
  ...
  if (!prod(dim(metrics_known_to_be_in_functions2))) {
    ...
    what_i_really_need <-
      merge(what_i_need, metrics_known_to_be_in_functions2, by = "function_name") # nolint: line_length_linter.
  }

  to_add <- util_anti_join_common(
    what_i_really_need,
