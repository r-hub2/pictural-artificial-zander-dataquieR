...
util_replace_hard_limit_violations <- function(study_data,
  meta_data,
  label_col) {
  if (!isTRUE(util_attr(study_data, "Codes_to_NA", exact = TRUE))) {
    util_error(
      c(
        "Missing codes have to have been replaced already by NA, before %s",
  ...
  if (!isTRUE(util_attr(study_data, "MAPPED", exact = TRUE))) {
    util_error(
      c(
        "%s must have been mapped with %s, before %s",
        "violations can be replaced by NA. This is an internal error.",
        "As a dataquieR developer, please ensure, that you only pass",
    ...
    FUN = function(col, int, nm, dt) {
      if (inherits(int, "interval") && (prep_dq_data_type_of(col) %in%
        ...
        if (inherits(int, "interval")) {
          util_warning(
            c(
              "For %s, I have %s but the column is of type %s",
              "(metadata say %s)"
            ),
