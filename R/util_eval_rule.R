...
util_eval_rule <- function(rule, ds1, meta_data = "item_level",
  ...
  if (replace_limits && replace_missing_by != "NA") {
    util_message(
      c(
        "Cannot replace hard limits, if missing codes are not deleted. I will",
  ...
  if (replace_missing_by %in% c("LABEL", "INTERPRET")) {
    ...
    vars_value_label_tables <- setNames(
      nm = all.vars(rule),
      util_find_var_by_meta(
        resp_vars = all.vars(rule),
    ...
    not_yet_supported <- intersect(
      names(vars_data_type)[which(vars_data_type %in% c("float", "integer"))],
      union(
        names(vars_value_labels)[which(util_empty(vars_value_labels))],
        names(vars_value_label_tables)[which(util_empty(vars_value_label_tables))] # nolint: line_length_linter.
      )
    )
    if (length(not_yet_supported) > 0) {
      util_error(
        paste0(
          "Replacement of missing value codes by labels or ",
  ...
  if (!missing(ds1)) {
    problem <-
      isTRUE(util_attr(ds1, ("Codes_to_NA"), exact = TRUE)) &&
    ...
    if (!!prod(dim(meta_data))) {
      ds1_ready <-
        isTRUE(util_attr(ds1, "MAPPED", exact = TRUE)) &&
        identical(
          util_attr(ds1, "Codes_to_NA", exact = TRUE),
          replace_missing_by == "NA"
        ) &&
        identical(
          util_attr(ds1, "HL_viol_to_NA", exact = TRUE),
          replace_limits
        ) &&
        isTRUE(util_attr(ds1, "Data_type_matches", exact = TRUE))
      if (!ds1_ready) {
        ds1 <- prep_prepare_dataframes(
          .replace_hard_limits = replace_limits,
  ...
  if (!is.list(ds1)) {
    util_error(
      "%s must be an environment or a list (or a dataframe)",
      dQuote("ds1")
    )
  }
  ...
  if (use_value_labels || use_missing_labels) {
    util_stop_if_not(util_attr(ds1, "MAPPED", exact = TRUE))
    ...
    cols_with_valuelabels <- meta_data[
    ...
    ds1_with_labels[, cols_with_valuelabels] <- lapply(
      cols_with_valuelabels,
      function(cn) {
        if (replace_missing_by == "INTERPRET") {
          ...
          if (inherits(mltab, "try-error") ||
            ...
            util_warning(
              c(
                "For %s, I have no %s assignments from %s, so I will",
        ...
        util_stop_if_not(
          `Internal error, sorry, please report. VALUE_LABELS still in util_eval_rule()` = # nolint: line_length_linter.
            length(labs) == 0 ||
            all(util_empty(labs))
        )
        ...
        if (!use_value_labels) {
          labs <- paste(unique(ds1[[cn]]),
            collapse = sprintf(" %s ", SPLIT_CHAR)
          )
        }
        # if (all(util_empty(labs))) {
        labs <- as.character(labs)
        if (replace_missing_by == "INTERPRET" && !is.null(CODE_INTERPRET)) {
          value_labels <- lapply(labs, util_parse_assignments)
          conflicting_codes <- unique(unlist(lapply(value_labels, function(x) {
            intersect(names(x), as.character(unname(CODE_INTERPRET)))
          })))
          if (length(conflicting_codes) > 0) {
            util_message(
              c(
                "For %s, the codes %s have both a value label and a",
                "missing-code interpretation. The missing-code",
                "interpretation wins."
              ),
              dQuote(cn),
              dQuote(paste(conflicting_codes, collapse = ", ")),
              applicability_problem = TRUE
            )
            labs <- vapply(value_labels, function(x) {
              x <- x[!(names(x) %in% conflicting_codes)]
        ...
        if (replace_missing_by == "") { # keep the codes
        ...
        } else if (replace_missing_by == "LABEL") { # use the missing/jump labels # nolint: line_length_linter.
          jl <-
            prep_deparse_assignments(
              codes = jcodes,
              labels = jl
          ...
          if (!is.null(CODE_INTERPRET)) {
            ...
            ml <-
              prep_deparse_assignments(
                codes = CODE_INTERPRET,
                labels = names(CODE_INTERPRET)
              )
          } else { # keep the codes
            util_warning(
              "I do not have a %s with %s to use them for rules",
          ...
          # "NA" is already handled before by prep_prepare_dataframes with
          # .replace_missings
          jl <- ""
          ml <- ""
        }
        ...
        col_as_factor <-
          util_assign_levlabs(
            ...
            assignchar = "=",
            variable_name = cn,
            warn_if_inadmissible = FALSE
          ) # inadmissible categorical values are checked in an indicator, already # nolint: line_length_linter.
...
#' Evaluate a parsed REDCap rule without preparing study data again
#'
#' @param rule parsed REDCap rule
#' @param ds1 already prepared study data
#' @param meta_data metadata
#' @param label_col currently active metadata label column
#'
#' @return the result of the parsed rule
#'
#' @family redcap
#' @concept process
#' @noRd
util_eval_prepared_redcap_rule <- function(rule,
  ds1,
  meta_data = data.frame(),
  label_col =
    util_attr(ds1,
  ...
  redcap_rule_env <- util_get_redcap_rule_env()
  ds2 <- util_add_redcap_rule_label_aliases(
    ds1 = ds1,
    meta_data = meta_data,
    label_col = label_col
  )
  enclos <- new.env(parent = redcap_rule_env)
  enclos[["[meta_data]"]] <- meta_data # something, redcap cannot refert to, ensured by its strange name # nolint: line_length_linter.
  enclos[["[label_col]"]] <- label_col # something, redcap cannot refert to, ensured by its strange name # nolint: line_length_linter.
  # add metadata here
  res <- eval(
    expr = rule,
    envir = ds2,
...
#' Add alternative metadata labels as rule-evaluation aliases
#'
#' @param ds1 already prepared study data
#' @param meta_data metadata
#' @param label_col currently active metadata label column
#'
#' @return `ds1` amended with alias columns
#'
#' @family redcap
#' @concept process
#' @noRd
util_add_redcap_rule_label_aliases <- function(ds1,
  meta_data = data.frame(),
  label_col =
    util_attr(ds1,
      "label_col",
