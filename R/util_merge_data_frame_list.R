...
util_merge_data_frame_list <- function(data_frames, id_vars) {
  ...
  invisible(lapply(
    ...
    function(nm) {
      assign(
        nm, # ensure a speaking error message
        data_frames[[nm]]
      )
  ...
  for (acn in ambiguous_column_names) {
    ...
    all_equal <- function(cols, .res = res) {
      if (length(cols) < 2) {
        ...
        .res[[cols[[2]]]][is.na(.res[[cols[[2]]]])] <- # na.rm doesn't work, if recursion cuts off head and tail had an NA but later some value different from the old head # nolint: line_length_linter.
          .res[[cols[[1]]]][is.na(.res[[cols[[2]]]])]
        all(.res[, cols[[1]], drop = TRUE] == .res[, cols[[2]], drop = TRUE], na.rm = TRUE) && # nolint: line_length_linter.
          Recall(tail(cols, -1), .res = .res)
      }
    }
    if (all_equal(all_cns)) {
      res[[acn]] <- unlist(
        lapply(apply(res[, all_cns, drop = FALSE], 1, unique), setdiff, NA),
        recursive = FALSE
      )
      res[, all_cns] <- NULL
    }
