...
#'
#' @details
#' `resp_vars` is deprecated for this function. The function always addresses
#' all variables.
#'
...
prep_scalelevel_from_data_and_metadata <- function(resp_vars =
  ...
  md <- meta_data[
    ...
    intersect(
      c(
        label_col, SCALE_LEVEL, DATA_TYPE, UNIT,
  ...
  # Historical vectorized assignment parsing removed here.
  assignments <- lapply(
    meta_info[[VALUE_LABEL_TABLE]],
    function(vltnm) {
      if (vltnm %in% prep_list_dataframes()) {
        ...
        if (!(CODE_LABEL %in% colnames(rdf))) {
          rdf[[CODE_LABEL]] <- as.character(rdf[[CODE_VALUE]])
        }
  ...
  meta_info$is_ordered <- vapply(assignments, util_attr, "split_char",
  ...
  if (any(meta_info$is_ordered)) { # check for counter-intuitive ordered lists
    ...
    for (a in assignments[meta_info$is_ordered]) {
      if (all(suppressWarnings(is.na(as.integer(names(a)))) ==
        ...
        util_message(
          "Found counter-intuitive %s/%s: %s",
          VALUE_LABELS,
          VALUE_LABEL_TABLE,
          prep_deparse_assignments(
            labels = vapply(a, identity,
              FUN.VALUE = character(1)
            ),
            codes = names(a),
            split_char = "<"
  ...
  meta_info[
    util_empty(meta_info[[SCALE_LEVEL]]) &
      ...
      (!util_empty(meta_info[[VALUE_LABELS]]) | !util_empty(meta_info[[VALUE_LABEL_TABLE]])), SCALE_LEVEL # nolint: line_length_linter.
  ] <-
    ifelse(meta_info$is_ordered[util_empty(meta_info[[SCALE_LEVEL]]) &
      ...
      (!util_empty(meta_info[[VALUE_LABELS]]) | !util_empty(meta_info[[VALUE_LABEL_TABLE]]))], # nolint: line_length_linter.
    SCALE_LEVELS$ORDINAL,
    SCALE_LEVELS$NOMINAL
    )

  # identical(attr(util_parse_assignments("a < 2", split_char = c(SPLIT_CHAR,
  # "<")), "split_char"), "<")
