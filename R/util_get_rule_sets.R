...
util_get_rule_sets <- function() {
  ...
  if (!nzchar(shipped_rulesets) && # nocov start
  ...
  util_expect_data_frame(reftab, col_names = c("indicator_metric", GRADING_RULESET)) # nolint: line_length_linter.
  reftab <- reftab[!util_empty(reftab$indicator_metric), , drop = FALSE]
  reftab <- reftab[!util_empty(reftab[[GRADING_RULESET]]), , drop = FALSE]
  reftab$GRADING_RULESET <- suppressWarnings(trimws(reftab[[GRADING_RULESET]]))
  GRADING_RULESET_names <- setNames(nm = setdiff(unique(reftab[[GRADING_RULESET]]), NA)) # nolint: line_length_linter.
  ...
  GRADING_RULESETs <- lapply(GRADING_RULESET_names, function(rs) {
    reftab[!is.na(reftab$GRADING_RULESET) & reftab$GRADING_RULESET == rs, , drop = FALSE] # nolint: line_length_linter.
  })

  default_rs <- GRADING_RULESETs[["0"]]

  for (needed in setdiff(
    c(
      "CAT_applicability", "CAT_error",
    ...
    add_row <- shipped_rulesets[!is.na(shipped_rulesets$GRADING_RULESET) &
        ...
        shipped_rulesets$indicator_metric == needed, , drop = FALSE]
    default_rs <- util_rbind(
      default_rs,
      add_row
    )
  }
  ...
  GRADING_RULESETs[as.character(to_amend)] <- lapply(to_amend, function(rs) {
    ...
    dst <- default_rs
    dst <- dst[
      !(dst$indicator_metric %in% rst$indicator_metric), , # only deafult rules, if not overwritte in current # nolint: line_length_linter.
      drop = FALSE
    ]
