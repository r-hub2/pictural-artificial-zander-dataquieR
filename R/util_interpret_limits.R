...
util_interpret_limits <- function(mdata) {
  ...
  for (i in seq_along(lv)) {
    valid1 <- grepl(
      "[\\[(]([0-9\\.Ee+\\-]*|[\\+\\-]?Inf);([0-9\\.Ee+\\-]*|[\\+\\-]?Inf)[\\])]", # nolint: line_length_linter.
      gsub(" ", "", mdata[[lv[i]]], fixed = TRUE),
      perl = TRUE
    )
    valid2 <- vapply(mdata[[lv[i]]],
      FUN.VALUE = logical(1),
      function(x) {
        if (is.na(x) || trimws(x) == "") {
          return(FALSE)
        ...
        if (xs1 %in% c("", "Inf", "+Inf", "-Inf", "today")) {
          ...
          a <- !inherits(
            try(
              suppressWarnings(util_parse_date(xs1,
                  optional = FALSE
                )),
              silent = TRUE
            ),
          ...
          if (!a) {
            ...
            a <- !inherits(
              try(
                suppressWarnings(util_parse_time(xs1,
                    optional = FALSE
                  )),
                silent = TRUE
              ),
        ...
        if (xs2 %in% c("", "Inf", "+Inf", "-Inf", "today")) {
          ...
          b <- !inherits(
            try(
              suppressWarnings(util_parse_date(xs2,
                  optional = FALSE
                )),
              silent = TRUE
            ),
          ...
          if (!b) {
            ...
            b <- !inherits(
              try(
                suppressWarnings(util_parse_time(xs2,
                    optional = FALSE
                  )),
                silent = TRUE
              ),
    ...
    X <- mdata[valid & !(is.na(mdata[lv[i]])), c(VAR_NAMES, DATA_TYPE, lv[i]), drop = TRUE] # nolint: line_length_linter.
    ...
    if (any(valid)) {
      ...
      # extract values
      X_LOWER <- (gsub(X[, "1", drop = TRUE],
          replacement = "",
          pattern = "[\\(|\\[]",
          perl = TRUE
        ))
      X_UPPER <- (gsub(X[, "2", drop = TRUE],
          replacement = "",
          pattern = "[\\)|\\]]",
          perl = TRUE
        ))
      ...
      # --- detect time-only (hms) style limits (heuristic if no explicit TIME
      # type) ---
      has_colon_lower <- grepl(":", X_LOWER, fixed = TRUE)
      has_colon_upper <- grepl(":", X_UPPER, fixed = TRUE)
      looks_like_date_lower <- grepl("\\b\\d{4}[-/]\\d{2}[-/]\\d{2}\\b", X_LOWER) # nolint: line_length_linter.
      looks_like_date_upper <- grepl("\\b\\d{4}[-/]\\d{2}[-/]\\d{2}\\b", X_UPPER) # nolint: line_length_linter.
      ...
      # If DATA_TYPES$TIME becomes available here, include it in time_vars.
      ...
      if (any(time_vars)) {
        ...
        lower_t <- suppressWarnings(util_parse_time(X_LOWER[time_vars], optional = TRUE)) # nolint: line_length_linter.
        upper_t <- suppressWarnings(util_parse_time(X_UPPER[time_vars], optional = TRUE)) # nolint: line_length_linter.
      ...
      X$LOWER[!date_vars & !time_vars] <- suppressWarnings(as.numeric(X_LOWER[!date_vars & !time_vars])) # nolint: line_length_linter.
      X$UPPER[!date_vars & !time_vars] <- suppressWarnings(as.numeric(X_UPPER[!date_vars & !time_vars])) # nolint: line_length_linter.
      ...
      X$INCL_LOWER <- ifelse(grepl(p1, X[, "1", drop = TRUE], fixed = TRUE), FALSE, TRUE) # nolint: line_length_linter.
      X$INCL_UPPER <- ifelse(grepl(p2, X[, "2", drop = TRUE], fixed = TRUE), FALSE, TRUE) # nolint: line_length_linter.

      X <- X[, c("VAR_NAMES", "LOWER", "UPPER", "INCL_LOWER", "INCL_UPPER"), drop = FALSE] # nolint: line_length_linter.
