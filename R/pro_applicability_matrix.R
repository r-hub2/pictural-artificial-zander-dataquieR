...
pro_applicability_matrix <- function(study_data,
  ...
  if (!missing(meta_data_segment) && !is.null(meta_data_segment)) {
    ...
    # Overwriting the argument makes missing() unusable here.
  ...
  # type fits or not
  dt_appl <- as.numeric(util_compare_meta_with_study(
  ...
  colnames(app_matrix_long) <- c(
    "VARIABLES", "STUDY_SEGMENT", "IMPLEMENTATION",
    "APP_SCORE"
  )

  # Order rows by int, com, con, and acc
  app_matrix_long_int <- app_matrix_long[
    grep(
      "int_",
  ...
  app_matrix_long_int <-
    app_matrix_long_int[order(app_matrix_long_int$IMPLEMENTATION), , drop = FALSE] # nolint: line_length_linter.


  app_matrix_long_com <- app_matrix_long[
    grep(
      "com_",
      app_matrix_long$IMPLEMENTATION
    ), ,
  ...
  app_matrix_long_com <-
    app_matrix_long_com[order(app_matrix_long_com$IMPLEMENTATION), , drop = FALSE] # nolint: line_length_linter.

  app_matrix_long_con <- app_matrix_long[
    grep(
      "con_",
      app_matrix_long$IMPLEMENTATION
    ), ,
  ...
  app_matrix_long_con <-
    app_matrix_long_con[order(app_matrix_long_con$IMPLEMENTATION), , drop = FALSE] # nolint: line_length_linter.

  app_matrix_long_acc <- app_matrix_long[
    grep(
      "acc_",
      app_matrix_long$IMPLEMENTATION
    ), ,
  ...
  app_matrix_long_acc <-
    app_matrix_long_acc[order(app_matrix_long_acc$IMPLEMENTATION), , drop = FALSE] # nolint: line_length_linter.

  app_matrix_long <- rbind(
    app_matrix_long_int, app_matrix_long_com,
  ...
  # Historical inverse-score transformation removed in commit 214dd76a7d.

  ReportSummaryTable$N <- 4

  ReportSummaryTable <- util_set_report_summary_table_higher_means(
    ReportSummaryTable, "better"
  )
  ReportSummaryTable <- util_set_report_summary_table_continuous(
    ReportSummaryTable, FALSE
  )
  ReportSummaryTable <- util_set_report_summary_table_colcode(
    ReportSummaryTable, colcode
  )
  ReportSummaryTable <- util_set_report_summary_table_level_names(
    ReportSummaryTable,
