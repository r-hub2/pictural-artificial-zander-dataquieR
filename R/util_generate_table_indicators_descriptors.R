...
#'
#' @noRd
util_generate_table_indicators_descriptors <- function(report, repsum = NULL) {
  ...
  withr::with_environment(this, {
    ...
    if (!prod(dim(this$result)) || all(
      ...
      stopped_functions <- cbind(
        rownames(stopped_functions),
      ...
      stopped_functions$fun_names1 <- gsub(
        "(^[^.]*).*", "\\1",
      ...
      stopped_functions <- unique(stopped_functions[, c(
        "fun_names1",
        "stopped_yn"
      ),
      drop = FALSE
      ...
      result_novars <- result_t[, c("call_names", "function_name"), drop = FALSE] # nolint: line_length_linter.
      ...
      # to use that with alias to see which function was not included in
      # result_t
      functions_in_result_t <- unique(result_novars$function_name2)
      rm(result_novars)


      ## Define which indicator_metric is an indicator based on
      #CAT_indicator_or_descriptor
      # select everything starting with CAT (Error category)
      result_err_cat <-
        result_t[
          startsWith(
            as.character(result_t$indicator_metric),
            "CAT_"
          ),
          c(
      ...
      # Historical dplyr rename variant removed here.
      names(result_err_cat)[names(result_err_cat) == "class"] <- "err_cat"

      # select only the category "CAT_indicator_or_descriptor"
      # Historical dplyr filter variant removed here.
      result_err_cat <- result_err_cat[
        result_err_cat$indicator_metric ==
          "CAT_indicator_or_descriptor", ,
        drop = FALSE
      ]

      # no need to reshape from long to wide, only CAT_indicator_or_descriptor
      # present
      # Historical reshape::cast variant removed here.
      ...
      # reduce no. columns
      result_err_cat <- result_err_cat[, c(
        VAR_NAMES, "call_names",
        "is_indicator"
      ), drop = FALSE]
      # remove rownames
      ...
      ## Remove all error messages (CAT and MSG) and obtain an object with
      #actual
      # results (metrics) only
      result_t <- result_t[!startsWith(
        as.character(result_t$indicator_metric),
        "CAT_"
      ), , drop = FALSE]
      result_t <- result_t[!startsWith(
      ...
      result_t <- result_t[!is.na(result_t$indicator_metric), , drop = FALSE]

      # remove indicators that have no class (class = NA)
      # Historical class-NA filter removed here.
      # remove indicators that have no value (value = NA)
      result_t <- result_t[!is.na(result_t$value), , drop = FALSE]

      # reduce no. columns to only needed
      result_t <- result_t[, c(VAR_NAMES, "indicator_metric", "is_indicator"), drop = FALSE] # nolint: line_length_linter.
      ...
      result_out$indicator_metric <- gsub(
        "(.*?_)(.*)", "\\2",
      ...
      result_out <- result_out[, c("indicator_metric", "is_indicator"), drop = FALSE] # nolint: line_length_linter.
      result_out <- result_out[!duplicated(result_out$indicator_metric), , drop = FALSE] # nolint: line_length_linter.

      # create a column with the DQ dimension of the indicator metric
      result_out$Dimension <- gsub(
        "(.*?)_.*", "\\1",
        vapply(result_out$indicator_metric,
          as.character,
          FUN.VALUE = character(1)
      ...
      # info_dim_dq <- suppressMessages(reshape::cast(
      #   info_dim_dq,
      #   Dimension ~ is_indicator
      # ))
      info_dim_dq <- stats::reshape(info_dim_dq,
        idvar = "Dimension",
        timevar = "is_indicator",
        direction = "wide"
      )

      colnames(info_dim_dq) <- gsub(
        "^n\\.",
        "",
      ...
      # merge the table with the no. indicators and the other with no.
      # descriptors
      info_dim_dq <- merge(info_dim_dq, n_descriptors,
        by = "Dimension",
        all = TRUE
      )
      rm(n_descriptors)
