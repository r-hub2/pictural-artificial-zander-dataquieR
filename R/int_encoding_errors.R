...
int_encoding_errors <- function(resp_vars = NULL,
  ...
  if (util_is_try_error(meta_data) || nrow(meta_data) == 0) {
    meta_data <- data.frame(
      ...
      DATA_TYPE = prep_datatype_from_data(
  ...
  ds1_label_col <- util_attr(resp_vars, "label_col", exact = TRUE)
  ...
  if (!any(resp_vars %in% resp_vars_character)) {
    util_error(
      c(
        "No %s are of type character, cannot work on",
    ...
    util_message(
      c(
        "Not all %s are of type character, leave out",
        "the non-character variables."
      ),
      sQuote("resp_vars"),
      applicability_problem = TRUE,
  ...
  if (missing(ref_encs)) {
    # Historical ref_enc extraction from verification results removed here.
    ref_encs <- util_get_encoding(
      study_data = ds1,
  ...
  cols_enc <- util_attr(res, "cols_enc", exact = TRUE)
  enc_guessed <- vapply(setNames(nm = names(cols_enc)), function(nm) {
    ...
    x <- x[order(x[["confidence"]], decreasing = TRUE), , drop = FALSE]
  ...
  pct_prolems_per_var <- 100 * num_prolems_per_var / nrow(study_data)
  SummaryTable <- data.frame(
    check.names = FALSE,
    Variables = resp_vars, # Historical label-column Variables fallback removed.
  ...
  attr(SummaryTable$`Defined Encoding`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryTable$`Guessed Encoding`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryTable$PCT_int_uenc, DATA_TYPE) <- DATA_TYPES$FLOAT
  attr(SummaryTable$NUM_int_uenc, DATA_TYPE) <- DATA_TYPES$INTEGER
  SummaryData <- SummaryTable[, 1:3, drop = FALSE]
  ...
  attr(SummaryData$`Defined Encoding`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryData$`Guessed Encoding`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryData$`Mismatches`, DATA_TYPE) <- DATA_TYPES$STRING
