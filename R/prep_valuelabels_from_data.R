...
prep_valuelabels_from_data <-
  function(resp_vars = colnames(study_data), study_data) {
    if (!missing(resp_vars) && is.data.frame(resp_vars) && missing(study_data)) { # nolint: line_length_linter.
      study_data <- resp_vars
      resp_vars <- colnames(study_data)
    } else if (missing(study_data) || !is.data.frame(study_data)) {
      util_error(
        "Need study data as a data frame in the argument %s",
        dQuote("study_data")
      )
    }
    ...
    factor_vars <-
      colnames(study_data)[vapply(study_data, is.factor, FUN.VALUE = logical(1))] # nolint: line_length_linter.
    ...
    valuelabels <- vapply(setNames(nm = resp_vars),
      ...
      function(variable) {
        if (variable %in% factor_resp_vars) {
          fctr <- factor(
            levels = levels(study_data[[variable]]),
    ...
    modifiedstudydata[, factor_resp_vars] <-
      lapply(study_data[, factor_resp_vars, drop = FALSE], as.integer)
