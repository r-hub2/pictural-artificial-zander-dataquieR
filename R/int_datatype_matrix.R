...
int_datatype_matrix <- function(resp_vars = NULL,
  ...
  if (length(max_vars_per_plot) != 1 ||
      (!util_is_integer(max_vars_per_plot) &&
          !is.infinite(max_vars_per_plot)) ||
  ...
  # Trigger-matrix generation is currently not used here.
  ...
  if (!(DATA_TYPE %in% names(meta_data))) {
    util_error(
      c(
        "The attribute DATA_TYPE is not contained in the metadata but is",
  ...
  if (any(is.na(meta_data[[DATA_TYPE]]))) {
    whichnot <- as.character(meta_data[[label_col]][is.na(meta_data[[DATA_TYPE]])]) # nolint: line_length_linter.
    util_error(
      paste0(
        "The DATA_TYPE for variable(s) <<", whichnot,
  ...
  # type fits or not
  dt_appl <-
    as.numeric(util_compare_meta_with_study(
      sdf = ds1, mdf = meta_data,
      label_col = label_col,
  ...
  app_matrix <- merge(app_matrix, meta_data[
    , intersect(
      c(
        VAR_NAMES, LABEL,
        STUDY_SEGMENT,
  ...
  app_matrix <- app_matrix[, !(names(app_matrix) %in% c(
    VAR_NAMES, LABEL,
    label_col
  )), drop = FALSE]
  ...
  if (VARIABLE_ORDER %in% colnames(meta_data)) {
    ...
    meta_data[[VARIABLE_ORDER]][is.na(meta_data[[VARIABLE_ORDER]])] <-
      1
    app_matrix <- app_matrix[na.omit(match(
      meta_data[[label_col]],
  ...
  app_matrix$Variables <- factor(app_matrix$Variables,
    levels =
      app_matrix$Variables
  )
  ...
  colnames(app_matrix_long) <- c(
    "VARIABLES", "SEGMENT", "IMPLEMENTATION",
    "APP_SCORE"
  )
  ...
  # PLOT -----------------------------------------------------------------------
  # Historical five-color palette changed in commit 05f83104ae.
    ...
    util_create_lean_ggplot(
      ggplot(m, aes(
        ...
        theme(
          ...
          # Historical aspect-ratio setting remains in commit 01e0267432.
        ),
      m = m,
  ...
  attr(pl, "sizing_hints") <- list(
    ...
    number_of_vars = length(lapply(
      split(app_matrix_long, app_matrix_long$SEGMENT),
      nrow
    )), # a list with the no. variables per segment
    number_of_cat = 3
  ) # this is always 3 types


  cmp_res <- util_compare_meta_with_study(
    sdf = ds1, mdf = meta_data,
    label_col = label_col,
    check_convertible = TRUE,
    check_conversion_stable = TRUE,
  ...
  wv <- util_attr(cmp_res, "which_vec", exact = TRUE)
  # Matching variables have no row-level diagnostics and therefore empty
  # vectors.
  wv <- wv[vapply(wv, length, FUN.VALUE = integer(1)) > 0]
  ...
  ReportSummaryTable <- util_set_report_summary_table_continuous(
    ReportSummaryTable, TRUE
  )
  ReportSummaryTable <- util_set_report_summary_table_colscale(
    ReportSummaryTable, c("#B2182B", "#92C5DE", "#2166AC")
  )

  ReportSummaryTable <- util_new_report_summary_table(ReportSummaryTable,
    meta_data = meta_data,
    label_col = label_col
  )
  ...
  app_matrix$PCT_int_vfe_type <-
    setNames(100 * ReportSummaryTable$`nonconvertible mismatch`,
      nm = ReportSummaryTable$Variables
    )[app_matrix$Variables]

  app_matrix$Variables <- as.character(app_matrix$Variables)

  # create SummaryData table
  SummaryData <- app_matrix[, c("Variables", "PCT_int_vfe_type"), drop = FALSE]

  SummaryData$PCT_int_vfe_type <-
    paste0(
      round(SummaryData$PCT_int_vfe_type * nrow(ds1) / 100, digits = 0),
      " (",
  ...
  #     "PCT_int_vfe_type", short = FALSE, long = FALSE
  #   )

  SummaryData$`Convertible mismatch, stable N (%)` <-
    paste0(
      round(ReportSummaryTable$`convertible mismatch, stable` * nrow(ds1),
        digits = 0
      ), " (",
      base::format(100 * ReportSummaryTable$`convertible mismatch, stable`, digits = 2, nsmall = 2), # nolint: line_length_linter.
      "%)"
    )
  SummaryData$`Convertible mismatch, unstable N (%)` <-
    paste0(
      round(ReportSummaryTable$`convertible mismatch, unstable` * nrow(ds1),
        digits = 0
      ), " (",
      base::format(100 * ReportSummaryTable$`convertible mismatch, unstable`, digits = 2, nsmall = 2), # nolint: line_length_linter.
      "%)"
    )

  SummaryData$`Data type match N (%)` <-
    paste0(
      round(match / 100 * nrow(ds1), digits = 0), " (",
  ...
  attr(SummaryTable$PCT_int_vfe_type, DATA_TYPE) <- DATA_TYPES$FLOAT
  attr(SummaryTable$CLS_int_vfe_type, DATA_TYPE) <- DATA_TYPES$INTEGER
  attr(SummaryTable$STUDY_SEGMENT, DATA_TYPE) <- DATA_TYPES$STRING
  ...
  ReportSummaryTable[
    ...
    round(ReportSummaryTable[, c(
      "convertible mismatch, stable",
      "convertible mismatch, unstable",
      "nonconvertible mismatch",
      "N"
    ), drop = FALSE] / ReportSummaryTable$N * nrow(ds1), digits = 0)
  ReportSummaryTable <- util_set_report_summary_table_relative(
    ReportSummaryTable, TRUE
  )

  if (!missing(orig_label_col)) { # map back ReportSummaryTable to requested label_col and tables to suitable table labels # nolint: line_length_linter.
    SummaryTable[["Variables"]] <-
      prep_map_labels(
        x = SummaryTable[["Variables"]],
        item_level = meta_data,
        to = orig_label_col,
        from = label_col,
    ...
    ReportSummaryTable[["Variables"]] <-
      prep_map_labels(
        x = ReportSummaryTable[["Variables"]],
        item_level = meta_data,
        to = orig_label_col,
        from = label_col,
        ifnotfound = SummaryTable[["Variables"]],
  ...
  ReportSummaryTable <- util_set_report_summary_table_flip_mode(
    ReportSummaryTable, "noflip"
  )
  ...
  attr(SummaryData$`Data type mismatch N (%)`, DATA_TYPE) <-
    DATA_TYPE_NUMBER_PAREN
  attr(SummaryData$`Convertible mismatch, stable N (%)`, DATA_TYPE) <-
    DATA_TYPE_NUMBER_PAREN
  attr(SummaryData$`Convertible mismatch, unstable N (%)`, DATA_TYPE) <-
    DATA_TYPE_NUMBER_PAREN
  attr(SummaryData$`Data type match N (%)`, DATA_TYPE) <-
    DATA_TYPE_NUMBER_PAREN
  attr(SummaryData$`Expected DATA_TYPE`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryData$`Observed DATA_TYPE`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryData$`State, given threshold`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryData$STUDY_SEGMENT, DATA_TYPE) <- DATA_TYPES$STRING
  attr(SummaryTable$Variables, DATA_TYPE) <- DATA_TYPES$STRING
  attr(ReportSummaryTable$`convertible mismatch, stable`, DATA_TYPE) <-
    DATA_TYPES$FLOAT
  attr(ReportSummaryTable$`convertible mismatch, unstable`, DATA_TYPE) <-
    DATA_TYPES$FLOAT
  attr(ReportSummaryTable$`nonconvertible mismatch`, DATA_TYPE) <-
    DATA_TYPES$FLOAT
  attr(ReportSummaryTable$N, DATA_TYPE) <- DATA_TYPES$INTEGER
