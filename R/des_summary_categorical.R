...
#' are considered and the result is restricted to nominal or ordinal variables.
#' Selected variables may include multiple variables, but each selected variable
#' must be nominal or ordinal scaled.
#'
...
#'
# nolint end
des_summary_categorical <- function(resp_vars = NULL,
  study_data,
  label_col,
  ...
  if (util_is_try_error(
    erobj <- try(prep_prepare_dataframes(
  ...
  # Historical resp_vars intersection with study-data columns removed here.


  # Select only resp_vars that are categorical
  df_category <- meta_data[meta_data[, label_col, drop = TRUE] %in% resp_vars, , drop = FALSE] # nolint: line_length_linter.
