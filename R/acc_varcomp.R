...
acc_varcomp <- function(resp_vars = NULL,
  ...
  if (lifecycle::is_present(threshold_value)) {
    ...
    lifecycle::deprecate_stop("2.5.0",
      ...
      details =
        c(
          "use the grading rules instaed, see",
  ...
  # Historical stricter group variable validation changed in commit 214dd76a7d.
  ...
  if (var_prop$NDistinct == 1) {
    ...
    var_scale %in% c("ordinal", "ratio", "interval"))) { ## binary response ---------------------------- # nolint: line_length_linter.
    ...
    check_df <- check_df[order(check_df$Freq, decreasing = TRUE), , drop = FALSE] # nolint: line_length_linter.
    ds1[[resp_vars]] <- ifelse(ds1[[resp_vars]] == check_df$Var1[1], 1, 0)
    rvs_bin_note <- paste(
      paste("Cases (1):", check_df[1, 1, drop = TRUE]),
      paste("Control (0):", check_df[2, 1, drop = TRUE]),
      sep = ". "
    )
    ...
    res_icc <- data.frame(
      Variables = resp_vars,
      # The historical category column is intentionally not emitted.
      Object = group_vars,
    ...
    # Historical response-category threshold check removed in commit c48791d1e3.
  ...
  } else if (var_scale == SCALE_LEVELS$ORDINAL && var_prop$NDistinct > 2) { ## ordinal response ---------- # nolint: line_length_linter.
    ...
    if (var_dtype == DATA_TYPES$INTEGER) {
      ...
      fit1 <- try(lme4::lmer(formula = fmla, data = ds1, REML = TRUE))
      # Historical Poisson and negative-binomial GLMM options remain visible in
      # commit 6be4b57a8e.
      icc <- NA
      if (!inherits(fit1, "try-error")) {
        # extract variance
      ...
      res_icc <- util_acc_varcomp(
        resp_vars = resp_vars, group_vars = group_vars,
        co_vars = co_vars,
      ...
      util_error("No method implemented for scale level ratio and data types string or datetime, sorry.", # nolint: line_length_linter.
        applicability_problem = TRUE
      )
    }
    ...
    if (var_dtype == DATA_TYPES$INTEGER) {
      ...
      util_error("No method implemented for scale level interval and data types string or datetime, sorry.", # nolint: line_length_linter.
        applicability_problem = TRUE
      )
    }
  }
  ...
  if (is.data.frame(res_icc)) {
    ...
    attr(sumtab$Variables, DATA_TYPE) <- DATA_TYPES$STRING
    attr(sumtab$ICC_acc_ud_loc, DATA_TYPE) <- DATA_TYPES$FLOAT
    attr(sumtab$Mean.Class.Size, DATA_TYPE) <- DATA_TYPES$FLOAT
    attr(sumtab$Median.Class.Size, DATA_TYPE) <- DATA_TYPES$FLOAT

    SummaryData <- res_icc

    SummaryData <- SummaryData[, setdiff(colnames(SummaryData), c("Model.Call")), drop = FALSE] # nolint: line_length_linter.
    attr(SummaryData$Variables, DATA_TYPE) <- DATA_TYPES$STRING
    attr(SummaryData$ICC, DATA_TYPE) <- DATA_TYPES$FLOAT
    attr(SummaryData$Mean.Class.Size, DATA_TYPE) <- DATA_TYPES$FLOAT
    attr(SummaryData$Median.Class.Size, DATA_TYPE) <- DATA_TYPES$FLOAT
