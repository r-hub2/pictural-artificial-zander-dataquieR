...
#'                             study data.
#' @param min_rows [numeric] minimum number of rows required.
#' @param empty_error_message [character] custom error message if `x` has fewer
#'                            than `min_rows` rows.
#' @param arg_name [character] user-facing argument name used in diagnostics.
...
util_expect_predicate_message <- function(check_type) {
  error_msg <- util_attr(check_type, "error_msg", exact = TRUE)
  if (!is.null(error_msg)) {
    return(error_msg)
  } else if (identical(check_type, is.character)) {
    return("be characters")
  } else if (identical(check_type, is.integer)) {
    return("be integer numbers")
  } else if (identical(check_type, is.double)) {
    return("be floating point")
  ...
  sprintf(
    "match the predicate %s",
    dQuote(paste(head(deparse(check_type)), collapse = " "))
  )
}

#' Internal helper: expect data frame
#'
#' @noRd
util_expect_data_frame <- function(x, col_names, convert_if_possible,
  ...
  util_expect_scalar(dont_assign,
    check_type = is.logical, error_message =
      sprintf(
        "%s needs to be to be a logical value.",
        sQuote("dont_assign")
      )
  )
  util_expect_scalar(min_rows,
    check_type = function(x) {
      is.numeric(x) && !is.na(x) && x >= 0
    },
    error_message =
      sprintf(
        "%s needs to be a non-negative number.",
        sQuote("min_rows")
      )
  )
  if (!missing(empty_error_message)) {
    util_expect_scalar(empty_error_message, check_type = is.character)
  }
  symb <- substitute(x)
  if (missing(arg_name)) {
    arg_name <- util_deparse1(symb)
  } else {
    util_expect_scalar(arg_name, check_type = is.character)
  }
  ...
  if (is.list(col_names)) {
    ...
    if (!length(names(lambdas))) {
      util_error(
        "%s needs to be a named list, if not a character vector",
        sQuote("col_names")
      )
    }
  ...
  if (missing(convert_if_possible)) {
    ...
    if (!length(names(convert_if_possible))) {
      util_error(
        "%s needs to be a named list, if a list",
    ...
    util_error(
      "%s needs to be a named list or NULL",
      sQuote("convert_if_possible")
    )
  }
  if (all(is.character(x)) && length(x) == 1) {
    ...
    data_frame_name <- x
    x <- try(prep_get_data_frame(x, keep_types = keep_types), silent = TRUE)
    if (util_is_try_error(x)) {
      util_error(
        c(
          "No data frame found for argument %s: %s.",
          "Load the data frame first or pass a data frame directly."
        ),
        sQuote(arg_name),
  ...
  if (!is.data.frame(x)) {
    util_error("%s is not a data frame.", sQuote(arg_name))
  }
  ...
  if (nrow(x) < min_rows) {
    if (!missing(empty_error_message)) {
      util_error("%s", empty_error_message, applicability_problem = TRUE)
    } else {
      util_error("Need at least %s rows in %s, got %s.",
  ...
  detect_mismatch <- function() {
    !vapply(names(lambdas[
      vapply(lambdas, is.function, FUN.VALUE = logical(1))
    ]), function(cl) {
      r <- lambdas[[cl]](x[[cl]])
      ...
      if (length(r) != 1 || is.na(r) || !is.logical(r)) {
        util_error(
          c(
            "Internal error in util_expect_data_frame:",
  ...
  if (any(mismatch)) {
    if (!missing(custom_errors)) {
      util_stop_if_not(
        "argument custom_errors needs to be a named list" =
          is.list(custom_errors)
      )
      cols_with_trouble <- names(which(mismatch))
      if (all(cols_with_trouble %in% names(custom_errors))) {
        util_error(
          applicability_problem = TRUE,
    ...
    mismatch_msg <- vapply(names(mismatch[mismatch]), function(cn) {
      sprintf(
        "Column %s in %s must %s.",
        sQuote(cn),
        sQuote(arg_name),
        util_expect_predicate_message(lambdas[[cn]])
      )
    }, FUN.VALUE = character(1))
    util_error("%s",
      paste(mismatch_msg, collapse = "\n"),
      applicability_problem = TRUE
    )
  }
  if (!dont_assign) {
