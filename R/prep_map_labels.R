...
#'   MISSING_LIST = ""
#' )
#' stopifnot(all(prep_map_labels(c("AGE", "DOE"), meta_data) == c(
#'   "Age",
#'   "Examination Date"
#' )))
#' }
prep_map_labels <- function(x, item_level = "item_level",
  ...
  # Metadata normalization belongs in util_prepare_item_level_metadata(); labels
  # have not changed enough here to justify re-preparing on every mapping call.
  # nocov start
  if (!is.data.frame(meta_data)) {
    util_error(
      c(
        "Need metadata as a data frame for mapping",
  ...
  if (!is.character(from) || length(from) != 1 ||
    ...
    util_error(
      c(
        "Need exactly one variable attribute name",
