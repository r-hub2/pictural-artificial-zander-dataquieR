...
#'                                        The default is set by the option
...
util_acc_varcomp <-
  function(resp_vars = NULL,
    ...
    if (is.na(.min_subgroups)) {
      util_message(
        "Could not convert min_subgroups %s to a number. Set to standard value.", # nolint: line_length_linter.
        dQuote(as.character(min_subgroups)),
        applicability_problem = TRUE
      )
      min_subgroups <- getOption(
        "dataquieR.min_group_var_levels",
        dataquieR.min_group_var_levels_default
      )
    } else {
    ...
    if (length(rvs) == 0) {
      rvs <- colnames(ds1[, vapply(ds1, is.numeric, FUN.VALUE = logical(1)), drop = FALSE]) # nolint: line_length_linter.
    ...
    if (length(group_vars) == 1 && length(rvs) > 1) {
      ...
      if (!.called_in_pipeline) {
        util_message(sprintf(
          "using the same group var %s for all resp_vars",
          dQuote(group_vars[[1]])
        ))
      }
    }
    ...
    if (length(group_vars) != length(rvs)) {
      util_error(
        c(
          "acc_varcomp expects one group_var per resp_var. Here,",
    ...
    ds1[, unique(group_vars)] <- lapply(
      ds1[, unique(group_vars), drop = FALSE],
      factor
    )
    ...
    icc_output <- apply(cbind(rvs, group_vars), 1, function(row) {
      ...
      if (length(critical_levels) > 0) {
        util_message("Levels %s were excluded due to fewer than %d observations.", # nolint: line_length_linter.
          paste0(
            c(
        ...
        ds1 <- ds1[!(ds1[[group_var]] %in% critical_levels), , drop = FALSE]
        # dropping unused levels
        ds1[[group_var]] <- factor(ds1[[group_var]])
      }

      check_df <-
        util_table_of_vct(ds1[[group_var]]) # number of observers may have changed # nolint: line_length_linter.
      ...
      res_df[1, "convergence.problem"] <- convergence_problem
      ...
      if (inherits(fit_lmer, "try-error")) {
        ...
        # calculate icc
        res_df[1, 4] <- round(v_tab$vcov[1] / (v_tab$vcov[1] + v_tab$vcov[2]), 3) # nolint: line_length_linter.
      }

      class_sizes <- util_table_of_vct(subdf[, group_var, drop = TRUE])
    ...
    attr(SummaryTable$ICC_acc_ud_loc, DATA_TYPE) <- DATA_TYPES$FLOAT
    attr(SummaryTable$Mean.Class.Size, DATA_TYPE) <- DATA_TYPES$FLOAT
    attr(SummaryTable$Median.Class.Size, DATA_TYPE) <- DATA_TYPES$FLOAT
