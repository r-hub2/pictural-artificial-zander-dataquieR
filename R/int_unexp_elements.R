...
int_unexp_elements <- function(identifier_name_list,
  ...
  if (missing(identifier_name_list) &&
    ...
    meta_data_dataframe <- meta_data_dataframe[
      vapply(meta_data_dataframe[[DF_NAME]],
        function(x) {
          !util_is_try_error(try(prep_get_data_frame(
            data_frame_name = x,
            keep_types = TRUE
          ), silent = TRUE))
        },
  ...
  util_stop_if_not(length(identifier_name_list) == length(data_element_count),
    label =
      sprintf(
        "In %s, %s and %s should have the same length: %s",
  ...
  result <- lapply(setNames(nm = identifier_name_list), function(current_df) {
    ...
    res_tmp <- data.frame(
      ...
      "Percentage of mismatches" =
        abs(round(100 * (data_elements - metadata_elements) / metadata_elements, 3)), # nolint: line_length_linter.
  ...
  attr(res_df$Check, DATA_TYPE) <- DATA_TYPES$STRING
  attr(res_df$`Data frame`, DATA_TYPE) <- DATA_TYPES$STRING
  attr(res_df$`Unexpected elements`, DATA_TYPE) <- DATA_TYPES$INTEGER
  attr(res_df$`Number of elements in data`, DATA_TYPE) <- DATA_TYPES$INTEGER
  attr(res_df$`Number of elements in metadata`, DATA_TYPE) <-
    DATA_TYPES$INTEGER
  attr(res_df$`Number of mismatches`, DATA_TYPE) <- DATA_TYPES$INTEGER
  attr(res_df$`Percentage of mismatches`, DATA_TYPE) <- DATA_TYPES$FLOAT
  attr(res_df$GRADING, DATA_TYPE) <- DATA_TYPES$INTEGER
  ...
  attr(res_pipeline$Level, DATA_TYPE) <- DATA_TYPES$STRING
  attr(res_pipeline$DF_NAME, DATA_TYPE) <- DATA_TYPES$STRING
  attr(res_pipeline$NUM_int_sts_countel, DATA_TYPE) <- DATA_TYPES$INTEGER
  attr(res_pipeline$PCT_int_sts_countel, DATA_TYPE) <- DATA_TYPES$FLOAT
  attr(res_pipeline$GRADING, DATA_TYPE) <- DATA_TYPES$INTEGER
