...
util_combine_list_report_summaries <- function(to_combine,
  ...
  if (length(to_combine) == 0) {
    ...
    summary <- to_combine[[1]]
    this <- util_attr(summary, "this", exact = TRUE)
    if (is.environment(this) && is.data.frame(this$meta_data_cross_item)) {
      this$meta_data_cross_item <- util_mark_contradiction_only_groups(
    ...
    this1 <- util_attr(to_combine[[1]], "this", exact = TRUE)
    this2 <- util_attr(to_combine[[2]], "this", exact = TRUE)

    l1 <- this1$label_col
    l2 <- this2$label_col
    while (!is.null(util_attr(l1, "orig_label_col", exact = TRUE))) {
      l1 <- util_attr(l1, "orig_label_col", exact = TRUE)
    }
    while (!is.null(util_attr(l2, "orig_label_col", exact = TRUE))) {
      l2 <- util_attr(l2, "orig_label_col", exact = TRUE)
    }
    if (l1 != l2) {
      util_error(
    ...
    if (!prod(dim(result_2)) ||
        all(startsWith(as.character(result_2$indicator_metric), "CAT_") |
            startsWith(as.character(result_2$indicator_metric), "MSG_"))) {
      # create an empty data frame
      result_2 <- data.frame()
    }

    ## import meta_data
    md1 <- this1$summary_meta_data
    if (is.null(md1)) md1 <- this1$meta_data
    md2 <- this2$summary_meta_data
    if (is.null(md2)) md2 <- this2$meta_data

    ## check labels_of_var_names_in_report
    if (length(this1$labels_of_var_names_in_report) == 0) {
    ...
    if (type == "unique_vars") {
      # add a column origin in both result data frames (works also in case
      # empty)
      ...
      labels_of_var_names_in_report <- c(
        this1$labels_of_var_names_in_report,
      ...
      if (!"new_label" %in% names(md1)) { # in case we are merging the first 2 summaries # nolint: line_length_linter.
        md1$origin <- paste0(this1$title)
        # new column for label_col with the origin followed by the original
        # label_col
      ...
      # In case of presence of duplicated rows (excluding origin and
      # VARIABLE_ROLE)
      meta_data <- meta_data[
        !duplicated(meta_data[,
          setdiff(
            colnames(meta_data),
            c(
              "origin",
      ...
      ordered_var_names <-
        ordered_var_names[match(
          unique(names(ordered_var_names)),
      ...
      ## rownames_of_report
      rownames_of_report <- unique(c(
        this1$rownames_of_report,
      ...
      if (any(orig1 %in% orig2)) {
        util_error(
          "Two reports can not have the same title %s",
          dQuote(origin_name2)
        )
      }
      ...
      if (!"..Origin" %in% names(result_1)) {
        ...
        df_map1$new_VAR_NAMES <- paste0(
          origin_name1, "-",
          df_map1$VAR_NAMES
        )
        df_map1$new_label_col <- paste0(
          origin_name1, "-",
          df_map1[, names(df_map1) %in%
              this1$label_col, drop = TRUE]
        )
      }

      df_map2 <- md2
      df_map2 <- df_map2[, names(df_map2) %in% c("VAR_NAMES", this2$label_col), drop = FALSE] # nolint: line_length_linter.
      df_map2$new_VAR_NAMES <- paste0(origin_name2, "-", df_map2$VAR_NAMES)
      df_map2$new_label_col <- paste0(
        origin_name2, "-",
        df_map2[, names(df_map2) %in%
            this2$label_col, drop = TRUE]
      )
      ...
      if (!"..Origin" %in% names(md1)) {
        ...
        if ("LONG_LABEL" %in% names(md1)) {
          md1 <- dplyr::mutate(md1, LONG_LABEL = paste(md1$..Origin, LONG_LABEL)) # nolint: line_length_linter.
        }
      }
      ...
      if (!"new_label" %in% names(md1)) {
        ...
        names(lab1) <- "new_label"
        md1 <- cbind(md1, lab1)
        md1 <- dplyr::mutate(md1, new_label = paste(
          md1$..Origin, "-",
          md1$new_label
        ))
        ...
        md1[[VAR_NAMES]] <- paste0(md1$..Origin, "-", md1[[VAR_NAMES]])
      ...
      names(lab1) <- "new_label"
      md2 <- cbind(md2, lab1)
      md2 <- dplyr::mutate(md2, new_label = paste(
        md2$..Origin, "-",
        md2$new_label
      ))
      ...
      md2[[VAR_NAMES]] <- paste0(md2$..Origin, "-", md2[[VAR_NAMES]])
      # merge the metadata
      meta_data <- merge(md1, md2, all = TRUE)
      attr(meta_data, "version") <- 2
      attr(meta_data, "normalized") <- TRUE
    ...
    alias_names <- alias_names[match(
      unique(names(alias_names)),
      names(alias_names)
    )]

    ## ordered_call_names
    ordered_call_names <- unique(c(
      this1$ordered_call_names,
      this2$ordered_call_names
    ))

    variable_group_call_names <- unique(c(
      this1$variable_group_call_names,
      this2$variable_group_call_names
    ))

    cross_item_metadata <- Filter(is.data.frame, list(
      this1$meta_data_cross_item,
      this2$meta_data_cross_item
    ))
    meta_data_cross_item <- if (length(cross_item_metadata)) {
      unique(util_rbind(data_frames_list = cross_item_metadata))
    } else {
      NULL
    }
    ...
    ## colnames_of_report
    colnames_of_report <- unique(c(
      this1$colnames_of_report,
    ...
    stopped_functions <-
      stopped_functions[match(
        unique(names(stopped_functions)),
    ...
    list_all <- as.data.frame(do.call(mapply, c(
      FUN = c,
      lapply(l_all, `[`, keys)
    )))
    filter_of <- lapply(
    ...
    this[["summary_meta_data"]] <- meta_data
