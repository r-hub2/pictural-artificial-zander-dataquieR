...
#'                                    by this factor.
#' @param repsum [data.frame] the `dataquieR` summary, see [summary()] and
#'               [dq_report2()]
...
util_html_for_dims <- function(report, use_plot_ly, template,
  ...
  meta_data_cross_item <- util_report_meta_data_cross_item(report)
  ...
  clls_in_rep_2 <-
    factor(vapply(strsplit(colnames(report), "_", fixed = TRUE), `[[`, 1,
        FUN.VALUE = character(1)
      ), names(dims), ordered = TRUE)
  sorted_clls <- colnames(report)[order(order(clls_in_rep_2))]

  i <- 0
  n <- ncol(report)

  # Historical parallelMap export/library setup handled earlier.
  ...
  for (cur_block in seq_len(nblocks) - 1) { # create the single variable pages
    ...
    combined_list <- Map(
      function(results, clls) list(results = results, clls = clls),
      results = results,
      clls = clls_in_chunk
    )

    chunk_of_pages <- util_par_lapply_lb(
      # Historical chunk.size override removed here.
      X = combined_list,
      fun = util_isolate_function(function(cml, ...) {
        ...
        cml$results <- # remove results for SSI part of the report
          cml$results[vapply(
            lapply(cml$results, util_attr, COMPUTED_VARIABLE_ROLE,
              exact = TRUE
            ),
            is.null,
            FUN.VALUE = logical(1)
...
#' Internal helper: html for dim
#'
#' @noRd
util_html_for_dim <- function(results, cll, repsum, function_alias_map,
  meta_data, meta_data_cross_item = NULL, label_col, use_plot_ly, dir,
  template, wd) {
  if (length(results) == 0) {
    return(NULL)
  }
  ...
  if (cur_dm %in% c("acc")) { # acc is in separate files
    fname <- paste0("dim_", cur_dm, "_", cll, ".html") # add the function name for accuracy # nolint: line_length_linter.
  } else {
    fname <- paste0("dim_", cur_dm, ".html") # define the file name, one file per dimension if not in accuracy # nolint: line_length_linter.
  }

  .fn <- paste0("dim_", cur_dm, "_", cll, ".RDS") # add the function name for accuracy # nolint: line_length_linter.
  ...
  sumplot <- plot(repsum,
    filter = call_names == cll, dont_plot = TRUE,
  ...
  resn <- intersect(
    resnames.dataquieR_resultset2(results),
  ...
  if (!any(resnames.dataquieR_resultset2(all_of_f) %in%
    ...
    if (length(resn) > 0) { # we have additional summaries to append
      ...
      all_of_f1 <- util_combine_res(
        lapply(results, util_subset_result_slot_for_combine, slot = resn)
      )
      all_of_f[[cll]][[resn]] <- all_of_f1[[cll]][[resn]]
    }
  }
  # create a page by iterating over all_of_f entries and their respective names
  # for each element the util_pretty_print function is called, converting each
  # dataquieR result to an htmltools compatible object
  # the do.call(htmltools::tagList) converts the output as an htmltools
  # compatible tag list ("as.tagList.list")
  # we have to unname the list of tags to prevent taglists from generating
  # named html attributes
  page <- do.call(htmltools::tagList, unname(mapply(
    dqr = all_of_f,
    nm = names(all_of_f),
    FUN =
      util_pretty_print,
  ...
  # check that the length of the page is positive and that it does not only
  # contain null
  if ((length(page) && !all(vapply(page, is.null, FUN.VALUE = logical(1))))) {
    if ("link" %in% unique(unlist(lapply(page, names)))) { # if we have page navigation links # nolint: line_length_linter.
      # extract link information for the page navigation menu (created by
      # util_pretty_print)
      ...
      all_links <- lapply(all_links, htmltools::tags$li) # add li to each tag to create the items # nolint: line_length_linter.
      all_links <- do.call(htmltools::tagList, all_links) # convert the list to a tag list, which is htmltools compatible # nolint: line_length_linter.
      page_menu <- util_float_index_menu(object = all_links) # create the menu
      # remove the original links (as created by util_pretty_print in the wrong
      # places)
      ...
      page <- htmltools::htmlTemplate(
        ...
        page = page,
        online_ref = online_refs$concept,
        online_ref_fallback = online_refs$implementation,
    ...
    # Check implementations and if there is a menu_location_report info, use it
    # to distribute results
    impls <- util_get_concept_info("implementations")
    if (fkt %in% impls$function_R &&
        impls[impls$function_R == fkt, "menu_location_report", drop = TRUE] != "") { # nolint: line_length_linter.
      drop_down <- impls[impls$function_R == fkt, "menu_location_report", drop = TRUE] # nolint: line_length_linter.
    }
