...
util_plot_categorical_vars <- function(resp_vars,
  ...
  if (length(tab_cat) > n_cat_max) {
    ...
    lvl_cat <-
      c(
        levels(plot_data[[resp_vars]])[which(levels(plot_data[[resp_vars]])
            %in% keep_cats)],
        "other"
      )
  ...
  if (!is.null(group_vars) && !is.null(time_vars)) {
    # Historical group-color scaling code removed here. Inspect commit
    # 214dd76a7d.
    pp <- util_histogram(
      plot_data = plot_data,
      is_datetime = TRUE,
    ...
    # Historical explicit color forwarding removed in commit 214dd76a7d.
    ...
    data_rep_sum_tab <- plot_data %>%
      dplyr::mutate(N = dplyr::n()) %>%
      dplyr::group_by(.data[[resp_vars]], .data[[group_vars]], N) %>%
      dplyr::summarise(freq = dplyr::n()) %>%
      dplyr::rename("Variables" = !!resp_vars) %>%
      as.data.frame(., check.names = FALSE)
    data_rep_sum_tab <- stats::reshape(data_rep_sum_tab,
      direction = "wide",
      idvar = c("Variables", "N"),
      timevar = group_vars
    )
    data_rep_sum_tab <- data_rep_sum_tab %>% replace(is.na(.), 0)
    colnames(data_rep_sum_tab) <- gsub("^freq\\.", "", colnames(data_rep_sum_tab)) # nolint: line_length_linter.
    class(data_rep_sum_tab) <- c("ReportSummaryTable", "data.frame")
    pp <- print(data_rep_sum_tab, flip_mode = "flip", view = FALSE)
    ...
    # Historical grouped plotly conversion hook removed in commit 214dd76a7d.
  } else {
    plot_data <- plot_data %>%
    ...
    pp <- util_bar_plot(
      ...
      num_var = "freq",
      show_numbers = FALSE
    ) # changed to FALSE to hide numbers
...
#' Internal helper: as plotly util plot categorical vars
#'
#' @noRd
util_as_plotly_util_plot_categorical_vars <- function(res) {
  ...
  black_bar_labs <-
    vapply(lapply(lapply(traces, `[[`, "textfont"), `[[`, "color"),
      identical, "rgba(0,0,0,1)",
      FUN.VALUE = logical(1)
    )
  white_bar_labs <-
    vapply(lapply(lapply(traces, `[[`, "textfont"), `[[`, "color"),
      identical, "rgba(255,255,255,1)",
      FUN.VALUE = logical(1)
    )
