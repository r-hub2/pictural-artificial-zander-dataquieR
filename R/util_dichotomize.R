...
#'
#' This function uses the metadata attributes `EVENT_LEVELS`/`CONTROL_LEVELS`
#' or their legacy aliases `RECODE_CASES`/`RECODE_CONTROL` to dichotomize the
#' data. 'Cases' will be recoded to 1, 'controls' to 0. The recoding can be
#' specified by an interval (for metric variables) or by a list of categories
#' separated by the 'SPLIT_CHAR'. Recoding will be used for data quality checks
#' that include a regression model.
#'
...
util_dichotomize <- function(study_data,
  meta_data,
  label_col = VAR_NAMES) {
  # Historical Codes_to_NA precondition check removed here.
  # would be a nice check, but the attribute gets lost when subsetting ds1,
  # e.g. to select certain columns and rows

  vrcnt <- 0
  list_out <- list()
  meta_data <- util_normalize_dichotomization_metadata(
    meta_data = meta_data,
    label_col = label_col
  )

  for (var in colnames(study_data)) {
    recode_cases <- meta_data[meta_data[[label_col]] == var, RECODE_CASES, drop = TRUE] # nolint: line_length_linter.
    recode_control <- meta_data[meta_data[[label_col]] == var, RECODE_CONTROL, drop = TRUE] # nolint: line_length_linter.
    case_vec <- NULL
    contr_vec <- NULL
    ...
    recode_uses_value_labels <- function(recode_vec) {
      length(recode_vec) > 0 && all(recode_vec %in% names(val_lab)) &&
        (val_lab_applied || any(
          as.character(study_data[[var]]) %in% unname(unlist(val_lab))
        ))
    }
    ...
    if ((!is.null(recode_cases) && !util_empty(recode_cases)) ||
      ...
      if (!is.null(recode_cases) && !util_empty(recode_cases)) {
        case_vec <- trimws(strsplit(recode_cases, SPLIT_CHAR, fixed = TRUE)[[1]]) # nolint: line_length_linter.
        case_vec <- case_vec[!util_empty(case_vec)]
        if (recode_uses_value_labels(case_vec)) {
          ...
          is_interval <- vapply(case_int,
            FUN.VALUE = logical(1),
            function(int) {
              inherits(int, "interval")
            }
          )
      ...
      if (!is.null(recode_control) && !util_empty(recode_control)) { # same as for cases above # nolint: line_length_linter.
        contr_vec <- trimws(strsplit(recode_control, SPLIT_CHAR, fixed = TRUE)[[1]]) # nolint: line_length_linter.
        contr_vec <- contr_vec[!util_empty(contr_vec)]
        if (recode_uses_value_labels(contr_vec)) {
      ...
      if (length(case_vec) == 0 && length(contr_vec) == 0) {
        ...
        if (length(case_vec) > 0) {
          if (is.list(case_vec)) {
            case_ind <- apply(
              as.data.frame(
                lapply(case_vec, function(ii) {
                  redcap_env$`in`(col, ii)
                })
              ),
              MARGIN = 1, FUN = any
            )
        ...
        if (length(c(which(contr_ind), which(case_ind))) == 0) {
          ...
          util_warning(paste(
            "Variable", var, "could not be recoded.",
...
#' Internal helper: normalize dichotomization metadata
#'
#' @noRd
util_normalize_dichotomization_metadata <- function(meta_data,
  label_col = VAR_NAMES) {
  if (!(RECODE_CASES %in% names(meta_data))) {
    meta_data[[RECODE_CASES]] <- ""
  }
  if (!(RECODE_CONTROL %in% names(meta_data))) {
  ...
  if (EVENT_LEVELS %in% names(meta_data)) {
    has_event_levels <- !vapply(
      meta_data[[EVENT_LEVELS]], util_empty,
      logical(1)
    )
    meta_data[[RECODE_CASES]][has_event_levels] <-
      meta_data[[EVENT_LEVELS]][has_event_levels]
  }

  if (CONTROL_LEVELS %in% names(meta_data)) {
    has_control_levels <- !vapply(
      meta_data[[CONTROL_LEVELS]], util_empty,
      logical(1)
    )
    meta_data[[RECODE_CONTROL]][has_control_levels] <-
      meta_data[[CONTROL_LEVELS]][has_control_levels]
  }
