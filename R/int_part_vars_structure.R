...
int_part_vars_structure <-
  function(label_col,
    ...
    pv <- meta_data[, PART_VAR, drop = TRUE]
    pv <- pv[!util_empty(pv)]
    if (length(pv) < nrow(meta_data)) {
      util_warning(
        "Found %d variables w/o %s in %s",
    ...
    r <- lapply(represening_rvs, function(rv) {
      ...
      if (any(missing_vars)) {
        util_warning(
          c(
            "Missing %s from %s, I fill it with NA.",
      ...
      # Historical slow PART_VAR bitstring prototype removed here. Inspect
      # with `git show 01e0267432 -- R/int_part_vars_structure.R`.


      test_part_bits <- function(part_bits_vector) {
        # Historical inline examples removed here.
      ...
      if (any(structure_damaged)) {
        case_data <- case_data <- ds1[structure_damaged, , drop = FALSE]
        ...
        util_warning(
          c(
            "In %s, I found %d inconsistencies in %s hiearchy, an",
          ...
          dQuote(util_map_labels(rv, meta_data,
              from = label_col,
              to = STUDY_SEGMENT
            )),
