...
#'
#' @details
#' If `resp_vars` is missing or empty, all variables defined as nominal or
#' ordinal in the metadata are selected. `resp_vars` must then identify one
#' nominal or ordinal measurement variable with at least two distinct
#' non-missing values.
#'
#' `group_vars` is optional. If supplied, it must identify one nominal or
#' ordinal grouping variable that is not a float variable and has at least two
...
acc_cat_distributions <- function(resp_vars = NULL,
  ...
  if (length(resp_vars) == 0) {
    util_message(
      c(
        "All variables defined to be nominal and ordinal in the metadata are used", # nolint: line_length_linter.
    ...
    if (length(resp_vars) == 0) {
      util_error("No suitable variables were defined for acc_cat_distributions.", # nolint: line_length_linter.
        applicability_problem = TRUE
      )
    }
  }
  ...
  as_plotly <- util_attr(p, "as_plotly", exact = TRUE)

  # Historical ggtitle subtitle variant removed here.
  p <- p + util_create_lean_ggplot(
    ylab(gvlab),
    gvlab = gvlab
  )
  # Historical direct util_set_size() call removed here.

  attr(p, "as_plotly") <- NULL
  dont_util_adjust_geom_text_for_plotly <-
    util_attr(p, "dont_util_adjust_geom_text_for_plotly", exact = TRUE)
