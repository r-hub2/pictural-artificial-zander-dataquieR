...
util_no_value_labels <- function(resp_vars, meta_data, label_col, warn = TRUE,
  ...
  matching_vars <- vapply(
    FUN.VALUE = logical(1), X = setNames(nm = resp_vars),
    FUN = function(rv) {
      row <- meta_data[meta_data[[label_col]] == rv, , drop = TRUE]
      vl <- row[[VALUE_LABELS]]
      ...
      if (dt == DATA_TYPES$FLOAT) {
        ...
        return(
          (length(vl) == 0 || is.na(vl) || trimws(vl) == "") &&
            (length(vlt) == 0 || is.na(vlt) || trimws(vlt) == "") &&
            (length(svt) == 0 || is.na(svt) || trimws(svt) == "")
        )
      } else {
        return(FALSE)
      }
  ...
  if (stop && all(!matching_vars)) {
    ...
    util_message(
      c(
        "The variables %s are neither float nor integer",
