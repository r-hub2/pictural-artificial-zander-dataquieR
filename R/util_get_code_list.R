...
util_get_code_list <- function(x, code_name, split_char = SPLIT_CHAR, mdf,
  ...
  if (!(code_name %in% names(mdf))) {
    if (warning_if_no_list) {
      util_warning(
        c(
          "Metadata does not provide a column called %s",
  ...
  if (length(c_list) != 1 || is.na(c_list)) {
    if (warning_if_no_list) {
      util_warning(
        c(
          "Could not find %s for %s in the meta_data",
          "for replacing codes with NAs."
        ),
        dQuote(code_name),
        dQuote(x),
  ...
  if (length(cl) != 1) { # nocov start
    util_error(c(
      "Internal error: Have more than one codelist for a",
      "variable in the metadata -- this should not happen."
    ))
  } # nocov end

  # Older parsing split cl directly on the configured separator.
  ...
  if (DATA_TYPE %in% colnames(mdf) &&
    ...
    if (sum(is.na(res)) < sum(is.na(dt_res))) {
      if (warning_if_unsuitable_list) {
        util_warning(
          "Some codes (%s) were not datetime/assignment for %s: %s, these will be ignored", # nolint: line_length_linter.
          dQuote(code_name),
          dQuote(x),
    ...
    if (sum(is.na(res)) < sum(is.na(to_res))) {
      if (warning_if_unsuitable_list) {
        util_warning(
          "Some codes (%s) were not time/assignment for %s: %s, these will be ignored", # nolint: line_length_linter.
          dQuote(code_name),
          dQuote(x),
    ...
    if (sum(is.na(res)) < sum(is.na(numeric_res))) {
      if (warning_if_unsuitable_list) {
        util_warning(
          "Some codes (%s) were not numeric/assignment for %s: %s, these will be ignored", # nolint: line_length_linter.
          dQuote(code_name),
          dQuote(x),
